...
- 如果把「手写 call / apply / bind」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「手写 new 与 instanceof」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「手写 curry / compose / pipe」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「手写 EventEmitter（含 once / off / wildcards）」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「手写一个简版 ajax / 带超时与重试的 fetch」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「系统设计：10 万条数据的高性能虚拟列表」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「系统设计：秒传 + 断点续传 + 大文件上传组件」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「面试表达策略：先结论，后展开，再补边界」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「设计一个富文本编辑器」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「设计一个多人实时协作系统（光标 / 编辑 / 在线状态）」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「你遇到过最难调的一个 bug 是什么」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「手写 call / apply / bind」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [手写, this, 高频, 追问]
...
如果面试官追问：如果把「手写 call / apply / bind」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「手写 new 与 instanceof」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [手写, 原型, 高频, 追问]
...
如果面试官追问：如果把「手写 new 与 instanceof」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「手写 curry / compose / pipe」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [函数式, 手写, 追问]
...
如果面试官追问：如果把「手写 curry / compose / pipe」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「手写 EventEmitter」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [发布订阅, 手写, 高频, 追问]
...
如果面试官追问：如果把「手写 EventEmitter（含 once / off / wildcards）」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：原生 structuredClone 内部是怎么实现的
difficulty: 资深
tags: [手写, 高频, 对象, 追问]
...
如果面试官追问：原生 structuredClone 内部是怎么实现的
...
title: 追问：跨 iframe 的对象 deep clone 有什么坑
difficulty: 资深
tags: [手写, 高频, 对象, 追问]
parent: handwrite-deep-clone-circular
...
如果面试官追问：跨 iframe 的对象 deep clone 有什么坑
...
title: 追问：拷贝带 prototype 的对象怎么保留原型链
difficulty: 资深
tags: [手写, 高频, 对象, 追问]
parent: handwrite-deep-clone-circular
...
如果面试官追问：拷贝带 prototype 的对象（如 class 实例）怎么保留原型链
...
- 先把问题拉回「手写深拷贝（处理循环引用 + Symbol + 特殊对象）」的核心机制，说明这个追问考察的是落地边界、失败条件和方案取舍，而不是单点定义。
- 可以补充一个真实项目语境：上线前先约定输入输出、失败兜底和观测指标，避免只在 demo 场景下成立。
...
title: 追问：如果把「手写一个简版 ajax / 带超时与重试的 fetch」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [网络, 手写, 追问]
...
如果面试官追问：如果把「手写一个简版 ajax / 带超时与重试的 fetch」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「系统设计：10 万条数据的高性能虚拟列表」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [系统设计, 虚拟列表, 追问]
...
如果面试官追问：如果把「系统设计：10 万条数据的高性能虚拟列表」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 变高列表：维护 measured: Map，用累加偏移 + 二分查找定位
- 可以补充一个真实项目语境：上线前先约定输入输出、失败兜底和观测指标，避免只在 demo 场景下成立。
...
title: 追问：如果把「系统设计：秒传 + 断点续传 + 大文件上传组件」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [系统设计, 上传, 追问]
...
如果面试官追问：如果把「系统设计：秒传 + 断点续传 + 大文件上传组件」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：怎么把 source map 安全地放到内网做反解
difficulty: 资深
tags: [系统设计, 监控, 追问]
...
如果面试官追问：怎么把 source map 安全地放到内网做反解
...
title: 追问：session replay 的隐私字段是怎么屏蔽的
difficulty: 资深
tags: [系统设计, 监控, 追问]
parent: design-monitoring-sdk
...
如果面试官追问：session replay 的隐私字段是怎么屏蔽的
...
title: 追问：如果把「面试表达策略：先结论，后展开，再补边界」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 基础
tags: [表达, 面试, 软技能, 追问]
...
如果面试官追问：如果把「面试表达策略：先结论，后展开，再补边界」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「设计一个富文本编辑器」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
...
如果面试官追问：如果把「设计一个富文本编辑器」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「设计一个多人实时协作系统」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [实时协作, 系统设计, 追问]
...
如果面试官追问：如果把「设计一个多人实时协作系统（光标 / 编辑 / 在线状态）」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「你遇到过最难调的一个 bug 是什么」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [软实力, 经验, 高频, 追问]
...
如果面试官追问：如果把「你遇到过最难调的一个 bug 是什么」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
