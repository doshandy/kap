...
- 如果把「Vue2 与 Vue3 的设计差异总览」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Vue3 响应式系统的 track / trigger 是怎么工作的」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Scheduler、批量更新与 nextTick 的真实含义」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Vue 组件通信方案怎么选」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Pinia 与 Vue Router 4 的工程实践」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「KeepAlive、Teleport、Suspense、异步组件分别解决什么问题」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「render 函数、JSX 与自定义指令分别适合什么场景」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「生命周期、错误边界与调试钩子怎么用」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Vue 性能优化：v-once、v-memo、shallowRef、虚拟列表怎么配合」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「composables 设计规范：命名、参数、返回值与副作用」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Nuxt 3 的核心价值：SSR、SSG、Nitro、payload」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Vue 3.5 Vapor Mode 与无 VDOM 渲染」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Vue 项目大促前的性能体检清单」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Vue 3 的 Proxy 响应式相比 Vue 2 的 defineProperty 解决了什么」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Vue 3 组件之间通信有哪些方式」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Vue 3 异步组件 + Suspense 怎么做骨架屏与错误兜底」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Vue2 与 Vue3 的设计差异总览」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 基础
tags: [架构, 响应式, 迁移, 追问]
parent: vue2-vs-vue3
...
如果面试官追问：如果把「Vue2 与 Vue3 的设计差异总览」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：为什么 Vue 3 用 Proxy 而不是 defineProperty 还需要「特殊处理」集合
difficulty: 进阶
tags: [响应式, API, 追问]
...
如果面试官追问：为什么 Vue 3 用 Proxy 而不是 defineProperty 还需要「特殊处理」集合
...
- 先把问题拉回「reactive、ref、shallow、readonly、toRef 的选择策略」的核心机制，说明这个追问考察的是落地边界、失败条件和方案取舍，而不是单点定义。
- 可以补充一个真实项目语境：上线前先约定输入输出、失败兜底和观测指标，避免只在 demo 场景下成立。
...
如果面试官追问：ref 内部其实是怎么实现的（getter/setter + dep）
...
title: 追问：如果把「Vue3 响应式系统的 track / trigger 是怎么工作的」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [响应式, 原理, 追问]
...
如果面试官追问：如果把「Vue3 响应式系统的 track / trigger 是怎么工作的」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Scheduler、批量更新与 nextTick 的真实含义」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [渲染, 调度, 追问]
...
如果面试官追问：如果把「Scheduler、批量更新与 nextTick 的真实含义」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：watchEffect 和 watch 何时选哪个
difficulty: 基础
tags: [响应式, API, 追问]
...
如果面试官追问：watchEffect 和 watch 何时选哪个
...
title: 追问：怎么实现「立即执行 + 后续异步取消」的 watch
difficulty: 基础
tags: [响应式, API, 追问]
parent: computed-watch
...
如果面试官追问：怎么实现「立即执行 + 后续异步取消」的 watch（参考 useFetch）
...
title: 追问：computed 是 lazy 的吗？什么时候算「计算时机」
difficulty: 基础
tags: [响应式, API, 追问]
parent: computed-watch
...
如果面试官追问：computed 是 lazy 的吗？什么时候算「计算时机」
...
title: 追问：React 的 fiber 算法和 Vue 的 diff 思路有什么不同
difficulty: 资深
tags: [diff, 编译优化, 追问]
...
如果面试官追问：React 的 fiber 算法和 Vue 的 diff 思路有什么不同
...
如果面试官追问：patchFlag 在编译期是怎么决定的（举几个 flag）
...
title: 追问：LIS在 Vue diff 里解决什么问题
difficulty: 资深
tags: [diff, 编译优化, 追问]
parent: diff-optimization
...
如果面试官追问：LIS（最长递增子序列）在 Vue diff 里解决什么问题
...
title: 追问：defineProps 在 ts 模式下和 js 模式下有什么差异
difficulty: 资深
tags: [编译, SFC, 追问]
parent: sfc-compile
...
如果面试官追问：defineProps 在 ts 模式下和 js 模式下有什么差异
...
title: 追问：script setup 编译产物大致长什么样
difficulty: 资深
tags: [编译, SFC, 追问]
parent: sfc-compile
...
如果面试官追问：script setup 编译产物大致长什么样
...
title: 追问：如果把「Vue 组件通信方案怎么选」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [组件通信, 设计, 追问]
...
如果面试官追问：如果把「Vue 组件通信方案怎么选」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Pinia 与 Vue Router 4 的工程实践」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
...
如果面试官追问：如果把「Pinia 与 Vue Router 4 的工程实践」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「KeepAlive、Teleport、Suspense、异步组件分别解决什么问题」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [高级组件, SSR, 追问]
...
如果面试官追问：如果把「KeepAlive、Teleport、Suspense、异步组件分别解决什么问题」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「render 函数、JSX 与自定义指令分别适合什么场景」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
...
如果面试官追问：如果把「render 函数、JSX 与自定义指令分别适合什么场景」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「生命周期、错误边界与调试钩子怎么用」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
...
如果面试官追问：如果把「生命周期、错误边界与调试钩子怎么用」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Vue 性能优化：v-once、v-memo、shallowRef、虚拟列表怎么配合」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
...
如果面试官追问：如果把「Vue 性能优化：v-once、v-memo、shallowRef、虚拟列表怎么配合」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「composables 设计规范：命名、参数、返回值与副作用」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
...
如果面试官追问：如果把「composables 设计规范：命名、参数、返回值与副作用」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Nuxt 3 的核心价值：SSR、SSG、Nitro、payload」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [Nuxt, SSR, 追问]
...
如果面试官追问：如果把「Nuxt 3 的核心价值：SSR、SSG、Nitro、payload」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Vue 3.5 Vapor Mode 与无 VDOM 渲染」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [Vapor, 编译优化, 追问]
parent: vapor-mode
...
如果面试官追问：如果把「Vue 3.5 Vapor Mode 与无 VDOM 渲染」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Vue 项目大促前的性能体检清单」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [性能, Vue, 追问]
...
如果面试官追问：如果把「Vue 项目大促前的性能体检清单」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Vue 3 的 Proxy 响应式相比 Vue 2 的 defineProperty 解决了什么」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [响应式, Vue3, 追问]
...
如果面试官追问：如果把「Vue 3 的 Proxy 响应式相比 Vue 2 的 defineProperty 解决了什么」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Vue 3 组件之间通信有哪些方式」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 基础
tags: [组件, Vue3, 追问]
...
如果面试官追问：如果把「Vue 3 组件之间通信有哪些方式」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Vue 3 异步组件 + Suspense 怎么做骨架屏与错误兜底」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [异步, 性能, 追问]
...
如果面试官追问：如果把「Vue 3 异步组件 + Suspense 怎么做骨架屏与错误兜底」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
