...
- 如果把「Webpack、Rollup、esbuild、SWC 各自擅长什么」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「npm、yarn、pnpm 与 lockfile、peerDependencies 的本质」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「SemVer、Conventional Commits、Changesets 分别治理什么问题」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Source Map、Browserslist 与 Polyfill 策略」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Monorepo、workspace、project references 的组合打法」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「ESLint、Prettier、Husky、lint-staged、CI 的职责边界」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「前端库的产物设计：ESM/CJS/types/exports/sideEffects」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Webpack 5 Module Federation 的价值与边界」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「exports、subpath imports 与现代包入口设计」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「CI/CD 缓存、矩阵构建与门禁设计」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Monorepo 多包发版（Changesets / Nx Release / Turborepo）」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Webpack / Rollup / Vite / Rolldown / Turbopack 比较」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Webpack 与 Vite 在开发态、构建态的差异」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
followups: [webpack-to-vite-migration-followup-1, webpack-to-vite-migration-followup-2, webpack-to-vite-migration-followup-3, webpack-to-vite-migration-followup-4, webpack-to-vite-migration-followup-5, webpack-to-vite-migration-followup-6]
difficulty: 资深
...
followups: [vite-vs-webpack-deep-followup-1, vite-vs-webpack-deep-followup-2, vite-vs-webpack-deep-followup-3, vite-vs-webpack-deep-followup-4, vite-vs-webpack-deep-followup-5, vite-vs-webpack-deep-followup-6, vite-vs-webpack-deep-followup-7, vite-vs-webpack-deep-followup-8]
difficulty: 进阶
...
followups: [vite-go-and-rolldown-followup-1, vite-go-and-rolldown-followup-2, vite-go-and-rolldown-followup-3, vite-go-and-rolldown-followup-4, vite-go-and-rolldown-followup-5, vite-go-and-rolldown-followup-6]
difficulty: 资深
...
followups: [tsconfig-paths-to-bundler-alias-followup-1, tsconfig-paths-to-bundler-alias-followup-2, tsconfig-paths-to-bundler-alias-followup-3, tsconfig-paths-to-bundler-alias-followup-4, tsconfig-paths-to-bundler-alias-followup-5, tsconfig-paths-to-bundler-alias-followup-6]
difficulty: 进阶
...
title: 追问：为什么 Vite 的 HMR 比 Webpack 快
difficulty: 进阶
tags: [Vite, 构建, 追问]
...
如果面试官追问：为什么 Vite 的 HMR 比 Webpack 快
...
title: 追问：Rolldown / Rspack / Turbopack 各自定位
difficulty: 进阶
tags: [Vite, 构建, 追问]
parent: vite-principle
...
如果面试官追问：Rolldown / Rspack / Turbopack 各自定位
...
- 先把问题拉回「Vite 为什么开发快、构建又能稳定」的核心机制，说明这个追问考察的是落地边界、失败条件和方案取舍，而不是单点定义。
- 可以补充一个真实项目语境：上线前先约定输入输出、失败兜底和观测指标，避免只在 demo 场景下成立。
...
title: 追问：esbuild 比 swc 快还是慢？哪个用得多
difficulty: 进阶
tags: [Vite, 构建, 追问]
parent: vite-principle
...
如果面试官追问：esbuild 比 swc 快还是慢？哪个用得多
...
title: 追问：如果把「Webpack、Rollup、esbuild、SWC 各自擅长什么」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
...
如果面试官追问：如果把「Webpack、Rollup、esbuild、SWC 各自擅长什么」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「npm、yarn、pnpm 与 lockfile、peerDependencies 的本质」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [pnpm, 依赖管理, 追问]
...
如果面试官追问：如果把「npm、yarn、pnpm 与 lockfile、peerDependencies 的本质」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「SemVer、Conventional Commits、Changesets 分别治理什么问题」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
...
如果面试官追问：如果把「SemVer、Conventional Commits、Changesets 分别治理什么问题」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Source Map、Browserslist 与 Polyfill 策略」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
...
如果面试官追问：如果把「Source Map、Browserslist 与 Polyfill 策略」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Monorepo、workspace、project references 的组合打法」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [Monorepo, TS, 追问]
parent: monorepo
...
如果面试官追问：如果把「Monorepo、workspace、project references 的组合打法」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「ESLint、Prettier、Husky、lint-staged、CI 的职责边界」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 基础
tags: [规范, CI, 追问]
parent: lint-ci
...
如果面试官追问：如果把「ESLint、Prettier、Husky、lint-staged、CI 的职责边界」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「前端库的产物设计：ESM/CJS/types/exports/sideEffects」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [发布, 包设计, 追问]
...
如果面试官追问：如果把「前端库的产物设计：ESM/CJS/types/exports/sideEffects」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Webpack 5 Module Federation 的价值与边界」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
...
如果面试官追问：如果把「Webpack 5 Module Federation 的价值与边界」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「exports、subpath imports 与现代包入口设计」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
...
如果面试官追问：如果把「exports、subpath imports 与现代包入口设计」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「CI/CD 缓存、矩阵构建与门禁设计」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
...
如果面试官追问：如果把「CI/CD 缓存、矩阵构建与门禁设计」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Monorepo 多包发版」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [Monorepo, 发版, 追问]
...
如果面试官追问：如果把「Monorepo 多包发版（Changesets / Nx Release / Turborepo）」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Webpack / Rollup / Vite / Rolldown / Turbopack 比较」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [打包工具, Vite, 追问]
parent: bundler-deep
...
如果面试官追问：如果把「Webpack / Rollup / Vite / Rolldown / Turbopack 比较」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Webpack 与 Vite 在开发态、构建态的差异」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
...
如果面试官追问：如果把「Webpack 与 Vite 在开发态、构建态的差异」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：强依赖 webpack-only 插件；超大 monorepo 且 prod build 已优化到极限的；微前端框架强绑定 webpack 的
difficulty: 资深
tags: [Vite, Webpack, 迁移, 高频, 追问]
parent: webpack-to-vite-migration
...
如果面试官追问：强依赖 webpack-only 插件（如某些联邦模块方案）；超大 monorepo 且 prod build 已优化到极限的；微前端框架强绑定 webpack 的（旧 qiankun + webpack-loader 体系）
...
## webpack-to-vite-migration-followup-4

title: 追问：dev cold start P95 < 5s；HMR P95 < 300ms；CI 构建 -20% 以上视为成功
difficulty: 资深
tags: [Vite, Webpack, 迁移, 高频, 追问]
parent: webpack-to-vite-migration
...
如果面试官追问：dev cold start P95 < 5s；HMR P95 < 300ms；CI 构建 -20% 以上视为成功
...
## webpack-to-vite-migration-followup-5

title: 追问：第三方 lib 不发 ESM 怎么办
difficulty: 资深
tags: [Vite, Webpack, 迁移, 高频, 追问]
parent: webpack-to-vite-migration
...
## webpack-to-vite-migration-followup-6

title: 追问：optimizeDeps.include 让 esbuild 预构建；patch-package 改 main field；最差自己 fork 发 ESM 版
difficulty: 资深
tags: [Vite, Webpack, 迁移, 高频, 追问]
parent: webpack-to-vite-migration
...
如果面试官追问：`optimizeDeps.include` 让 esbuild 预构建；patch-package 改 main field；最差自己 fork 发 ESM 版
...
title: 追问：Go 原生并发；汇编级解析器；不做 babel 那种 plugin AST 多次重写；牺牲了部分语法兼容性
difficulty: 进阶
tags: [Vite, Webpack, 高频, 追问]
parent: vite-vs-webpack-deep
...
如果面试官追问：Go 原生并发；汇编级解析器；不做 babel 那种 plugin AST 多次重写；牺牲了部分语法兼容性
...
title: 追问：Vite 用 esbuild 处理依赖，为什么源码不也用 esbuild 而是各自插件
difficulty: 进阶
tags: [Vite, Webpack, 高频, 追问]
parent: vite-vs-webpack-deep
...
如果面试官追问：Vite 用 esbuild 处理依赖，为什么源码不也用 esbuild 而是各自插件？
...
title: 追问：源码要走 vue / svelte / jsx 编译，esbuild 不直接支持；esbuild 做依赖即可
difficulty: 进阶
tags: [Vite, Webpack, 高频, 追问]
parent: vite-vs-webpack-deep
...
如果面试官追问：源码要走 vue / svelte / jsx 编译，esbuild 不直接支持；esbuild 做依赖即可
...
## vite-vs-webpack-deep-followup-6

title: 追问：文件级 invalidation；不需要 rebundle；websocket 推送精确边界
difficulty: 进阶
tags: [Vite, Webpack, 高频, 追问]
parent: vite-vs-webpack-deep
...
如果面试官追问：文件级 invalidation；不需要 rebundle；websocket 推送精确边界
...
## vite-vs-webpack-deep-followup-7

title: 追问：Vite prod 慢怎么优化
difficulty: 进阶
tags: [Vite, Webpack, 高频, 追问]
parent: vite-vs-webpack-deep
...
## vite-vs-webpack-deep-followup-8

title: 追问：关闭 sourcemap；manualChunks 控制 chunk 数；预构建大依赖
difficulty: 进阶
tags: [Vite, Webpack, 高频, 追问]
parent: vite-vs-webpack-deep
...
如果面试官追问：关闭 sourcemap；manualChunks 控制 chunk 数；预构建大依赖（rolldown 即将主线）
...
title: 追问：Rolldown 跟 Rollup 兼容到什么程度
difficulty: 资深
...
如果面试官追问：Rolldown 跟 Rollup 兼容到什么程度？
...
title: 追问：大部分插件兼容；某些钩子和 source map 处理有差异
difficulty: 资深
tags: [Vite, esbuild, Rolldown, 追问]
parent: vite-go-and-rolldown
...
如果面试官追问：大部分插件兼容（按 plugin API 接入）；某些钩子和 source map 处理有差异
...
title: 追问：为什么不直接用 esbuild 做 prod
difficulty: 资深
tags: [Vite, esbuild, Rolldown, 追问]
parent: vite-go-and-rolldown
...
如果面试官追问：为什么不直接用 esbuild 做 prod？
...
title: 追问：esbuild tree-shake / chunk 优化不如 Rollup；ecosystem 插件不够；体积优化弱
difficulty: 资深
tags: [Vite, esbuild, Rolldown, 追问]
parent: vite-go-and-rolldown
...
如果面试官追问：esbuild tree-shake / chunk 优化不如 Rollup；ecosystem 插件不够；体积优化弱
...
title: 追问：Oxc 和 Rolldown 是同一团队吗
difficulty: 资深
tags: [Vite, esbuild, Rolldown, 追问]
parent: vite-go-and-rolldown
...
## vite-go-and-rolldown-followup-6

title: 追问：都在 VoidZero下，但 Oxc 主要由 Boshen 推动，定位是 babel 替代
difficulty: 资深
tags: [Vite, esbuild, Rolldown, 追问]
parent: vite-go-and-rolldown
...
如果面试官追问：都在 VoidZero（尤雨溪牵头）下，但 Oxc 主要由 Boshen 推动，定位是 babel 替代
...
title: 追问：monorepo 里 tsconfig 多达几十个，怎么不重复
difficulty: 进阶
...
如果面试官追问：monorepo 里 tsconfig 多达几十个，怎么不重复？
...
title: 追问：根 tsconfig.base.json + 子包 extends；alias 基于 root 解析
difficulty: 进阶
tags: [配置, alias, monorepo, 追问]
parent: tsconfig-paths-to-bundler-alias
...
如果面试官追问：根 tsconfig.base.json + 子包 extends；alias 基于 root 解析
...
title: 追问：alias 如何和 Node.js 运行时保持一致
difficulty: 进阶
tags: [配置, alias, monorepo, 追问]
parent: tsconfig-paths-to-bundler-alias
...
如果面试官追问：alias 如何和 Node.js 运行时（如 SSR / vitest）保持一致？
...
title: 追问：用 tsx / tsconfig-paths/register 让 Node 也认 tsconfig paths
difficulty: 进阶
tags: [配置, alias, monorepo, 追问]
parent: tsconfig-paths-to-bundler-alias
...
如果面试官追问：用 `tsx` / `tsconfig-paths/register` 让 Node 也认 tsconfig paths
...
## tsconfig-paths-to-bundler-alias-followup-6

title: 追问：重启 ts server；检查 baseUrl + paths 拼接
difficulty: 进阶
tags: [配置, alias, monorepo, 追问]
parent: tsconfig-paths-to-bundler-alias
...
如果面试官追问：重启 ts server；检查 baseUrl + paths 拼接
