...
- 「AI 改写 SQL 的 Diff 接受/拒绝交互怎么做」上线前你会如何做效果评估、成本预算和安全防护？
...
title: 追问：推动「设计一个浏览器内的 SQL 工作台，整体架构怎么拆」落地时，你会如何设计灰度、回滚和迁移路径
difficulty: 资深
...
如果面试官追问：推动「设计一个浏览器内的 SQL 工作台，整体架构怎么拆」落地时，你会如何设计灰度、回滚和迁移路径？
...
title: 追问：你会先看哪些指标来判断「Monaco 多 Tab 编辑器实例之间怎么做隔离」是不是当前性能瓶颈
difficulty: 资深
...
如果面试官追问：你会先看哪些指标来判断「Monaco 多 Tab 编辑器实例之间怎么做隔离」是不是当前性能瓶颈？
...
title: 追问：你会先看哪些指标来判断「SQL 自动补全 / 校验怎么做才不卡 UI」是不是当前性能瓶颈
difficulty: 资深
...
如果面试官追问：你会先看哪些指标来判断「SQL 自动补全 / 校验怎么做才不卡 UI」是不是当前性能瓶颈？
...
title: 追问：你会先看哪些指标来判断「元数据接口高并发去重 + 缓存怎么设计」是不是当前性能瓶颈
difficulty: 进阶
tags: [缓存, 并发, 高频, 追问]
...
如果面试官追问：你会先看哪些指标来判断「元数据接口高并发去重 + 缓存怎么设计」是不是当前性能瓶颈？
...
title: 追问：「多 Tab 编辑器状态怎么持久化（投影模式）」在真实项目里最容易踩到哪些边界条件
difficulty: 资深
...
如果面试官追问：「多 Tab 编辑器状态怎么持久化（投影模式）」在真实项目里最容易踩到哪些边界条件？

### 答案要点

#### 核心回答

- 先界定「多 Tab 编辑器状态怎么持久化（投影模式）」的适用条件，再说明哪些输入、运行环境或团队约束会让默认方案失效。
- 围绕核心机制展开：正常路径要能讲清楚，异常路径要说明降级、替代方案和用户可见影响。
- 原题中的关键点「为什么不用 localStorage」要进一步补到边界条件里，而不是只复述结论。

## sql-result-polling-followup-1

title: 追问：你会先看哪些指标来判断「长 SQL 异步执行 + 前端轮询结果怎么设计」是不是当前性能瓶颈
difficulty: 进阶
tags: [轮询, 异步, 性能, 追问]
...
如果面试官追问：你会先看哪些指标来判断「长 SQL 异步执行 + 前端轮询结果怎么设计」是不是当前性能瓶颈？
...
title: 追问：「AI Agent 流式对话怎么渲染才不卡」上线前你会如何做效果评估、成本预算和安全防护
difficulty: 资深
...
如果面试官追问：「AI Agent 流式对话怎么渲染才不卡」上线前你会如何做效果评估、成本预算和安全防护？
...
title: 追问：「AI 改写 SQL 的 Diff 接受/拒绝交互怎么做」上线前你会如何做效果评估、成本预算和安全防护
difficulty: 资深
...
如果面试官追问：「AI 改写 SQL 的 Diff 接受/拒绝交互怎么做」上线前你会如何做效果评估、成本预算和安全防护？
...
title: 追问：「任务调度 DAG 依赖图怎么前端展示和交互」有哪些容易漏掉的边界输入和复杂度陷阱
difficulty: 资深
tags: [可视化, DAG, 调度, 追问]
...
如果面试官追问：「任务调度 DAG 依赖图怎么前端展示和交互」有哪些容易漏掉的边界输入和复杂度陷阱？
...
title: 追问：推动「多国 / 多环境部署（CN / ID / SP / MX）怎么管理差异」落地时，你会如何设计灰度、回滚和迁移路径
difficulty: 进阶
tags: [架构, 部署, i18n, 追问]
...
如果面试官追问：推动「多国 / 多环境部署（CN / ID / SP / MX）怎么管理差异」落地时，你会如何设计灰度、回滚和迁移路径？
...
title: 追问：如果把「复杂权限体系（数据 + 操作）前端怎么做」放到真实业务里，你会怎么划分信任边界和服务端兜底
difficulty: 资深
tags: [权限, 架构, 高频, 追问]
...
如果面试官追问：如果把「复杂权限体系（数据 + 操作）前端怎么做」放到真实业务里，你会怎么划分信任边界和服务端兜底？
...
title: 追问：你会先看哪些指标来判断「数据平台几十万行结果集表格怎么不卡」是不是当前性能瓶颈
difficulty: 资深
...
如果面试官追问：你会先看哪些指标来判断「数据平台几十万行结果集表格怎么不卡」是不是当前性能瓶颈？
...
title: 追问：「大文件分片上传怎么实现」在真实项目里最容易踩到哪些边界条件
difficulty: 进阶
tags: [上传, 分片, 高频, 追问]
...
如果面试官追问：「大文件分片上传怎么实现」在真实项目里最容易踩到哪些边界条件？

### 答案要点

#### 核心回答

- 先界定「大文件分片上传怎么实现」的适用条件，再说明哪些输入、运行环境或团队约束会让默认方案失效。
- 围绕核心机制展开：正常路径要能讲清楚，异常路径要说明降级、替代方案和用户可见影响。
- 原题中的关键点「1. 前端选文件后用 Web Worker 计算 hash（spark-md5）」要进一步补到边界条件里，而不是只复述结论。

## g2-charts-perf-followup-1

title: 追问：你会先看哪些指标来判断「数据看板（Dashboard）几十个图表同时渲染怎么不卡」是不是当前性能瓶颈
difficulty: 进阶
tags: [图表, 看板, 性能, 追问]
...
如果面试官追问：你会先看哪些指标来判断「数据看板（Dashboard）几十个图表同时渲染怎么不卡」是不是当前性能瓶颈？
...
title: 追问：推动「前端版本灰度 + 回滚怎么做」落地时，你会如何设计灰度、回滚和迁移路径
difficulty: 进阶
tags: [发布, 灰度, 工程化, 追问]
...
如果面试官追问：推动「前端版本灰度 + 回滚怎么做」落地时，你会如何设计灰度、回滚和迁移路径？
...
title: 追问：推动「系统设计题：从 0 设计一个数据平台前端，你怎么拆」落地时，你会如何设计灰度、回滚和迁移路径
difficulty: 资深
tags: [系统设计, 架构, 高频, 追问]
...
如果面试官追问：推动「系统设计题：从 0 设计一个数据平台前端，你怎么拆」落地时，你会如何设计灰度、回滚和迁移路径？
