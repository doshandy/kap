...
followups: [message-reliability-followup-1]
difficulty: 资深
tags: [IM, 可靠性, ack, 高频]
...
followups: [unread-count-sync-followup-1]
difficulty: 进阶
tags: [IM, 未读, 多端同步, 高频]
...
followups: [typing-presence-indicator-followup-1]
difficulty: 进阶
...
followups: [intl-deployment-region-followup-1]
difficulty: 资深
...
title: 追问：推动「IM 消息协议怎么设计？关键字段有哪些」落地时，你会如何设计灰度、回滚和迁移路径
difficulty: 进阶
tags: [IM, 协议, 高频, 追问]
...
如果面试官追问：推动「IM 消息协议怎么设计？关键字段有哪些」落地时，你会如何设计灰度、回滚和迁移路径？
...
title: 追问：「长连接的心跳保活和断线重连怎么做」在弱网、代理、断连或服务端限流时会出现哪些边界问题
difficulty: 进阶
...
如果面试官追问：「长连接的心跳保活和断线重连怎么做」在弱网、代理、断连或服务端限流时会出现哪些边界问题？
...
title: 追问：「消息可靠性（不丢、不重、有序）怎么保证」在真实项目里最容易踩到哪些边界条件
difficulty: 资深
...
如果面试官追问：「消息可靠性（不丢、不重、有序）怎么保证」在真实项目里最容易踩到哪些边界条件？
...
title: 追问：「多端未读计数怎么做才不会"标已读了红点还在"」在真实项目里最容易踩到哪些边界条件
difficulty: 进阶
...
如果面试官追问：「多端未读计数怎么做才不会"标已读了红点还在"」在真实项目里最容易踩到哪些边界条件？

### 答案要点

#### 核心回答

- 先界定「多端未读计数怎么做才不会"标已读了红点还在"」的适用条件，再说明哪些输入、运行环境或团队约束会让默认方案失效。
- 围绕核心机制展开：正常路径要能讲清楚，异常路径要说明降级、替代方案和用户可见影响。
- 原题中的关键点「核心原则：服务端是唯一真实来源，客户端不要自己累加未读数」要进一步补到边界条件里，而不是只复述结论。

## message-pagination-history-followup-1

title: 追问：你会先看哪些指标来判断「历史消息分页和会话首屏加载怎么设计」是不是当前性能瓶颈
difficulty: 进阶
tags: [IM, 分页, 缓存, 高频, 追问]
...
如果面试官追问：你会先看哪些指标来判断「历史消息分页和会话首屏加载怎么设计」是不是当前性能瓶颈？
...
title: 追问：「"对方正在输入" / 在线状态 / 已读回执 高频信令怎么做」在真实项目里最容易踩到哪些边界条件
difficulty: 进阶
...
如果面试官追问：「"对方正在输入" / 在线状态 / 已读回执 高频信令怎么做」在真实项目里最容易踩到哪些边界条件？
...
title: 追问：如果把「聊天消息支持富文本（链接 / 表情 / @ / 卡片 / 图片），怎么渲染才安全」放到真实业务里，你会怎么划分信任边界和服务端兜底
difficulty: 资深
...
如果面试官追问：如果把「聊天消息支持富文本（链接 / 表情 / @ / 卡片 / 图片），怎么渲染才安全」放到真实业务里，你会怎么划分信任边界和服务端兜底？
...
title: 追问：如果把「客服聊天的文件 / 图片上传：断点续传 + 缩略图 + 安全检查」放到真实业务里，你会怎么划分信任边界和服务端兜底
difficulty: 资深
...
如果面试官追问：如果把「客服聊天的文件 / 图片上传：断点续传 + 缩略图 + 安全检查」放到真实业务里，你会怎么划分信任边界和服务端兜底？
...
title: 追问：推动「智能客服路由：机器人优先 / 转人工 / 排队 / 坐席分配 怎么设计」落地时，你会如何设计灰度、回滚和迁移路径
difficulty: 资深
tags: [客服, 路由, 调度, 高频, 追问]
...
如果面试官追问：推动「智能客服路由：机器人优先 / 转人工 / 排队 / 坐席分配 怎么设计」落地时，你会如何设计灰度、回滚和迁移路径？
...
title: 追问：如果把「端到端加密的客服 IM 怎么实现？Web Crypto API 实战」放到真实业务里，你会怎么划分信任边界和服务端兜底
difficulty: 资深
...
如果面试官追问：如果把「端到端加密的客服 IM 怎么实现？Web Crypto API 实战」放到真实业务里，你会怎么划分信任边界和服务端兜底？
...
title: 追问：「海外客服系统多区域部署：怎么选接入点？怎么过合规」在真实项目里最容易踩到哪些边界条件
difficulty: 资深
...
如果面试官追问：「海外客服系统多区域部署：怎么选接入点？怎么过合规」在真实项目里最容易踩到哪些边界条件？

### 答案要点

#### 核心回答

- 先界定「海外客服系统多区域部署：怎么选接入点？怎么过合规」的适用条件，再说明哪些输入、运行环境或团队约束会让默认方案失效。
- 围绕核心机制展开：正常路径要能讲清楚，异常路径要说明降级、替代方案和用户可见影响。
- 原题中的关键点「多 region 部署网关：cn-shanghai / sea-singapore / us-east-1 / eu-frankfurt」要进一步补到边界条件里，而不是只复述结论。

## intl-time-locale-followup-1

title: 追问：「多时区 + 多语言客服会话：时间显示和消息排序怎么做不出错」有哪些容易漏掉的边界输入和复杂度陷阱
difficulty: 进阶
...
如果面试官追问：「多时区 + 多语言客服会话：时间显示和消息排序怎么做不出错」有哪些容易漏掉的边界输入和复杂度陷阱？
...
title: 追问：你会先看哪些指标来判断「海量消息聊天的虚拟列表怎么做？双向滚动 + 动态高度 + 贴底跟随」是不是当前性能瓶颈
difficulty: 资深
tags: [虚拟列表, 性能, 高频, 追问]
...
如果面试官追问：你会先看哪些指标来判断「海量消息聊天的虚拟列表怎么做？双向滚动 + 动态高度 + 贴底跟随」是不是当前性能瓶颈？
...
title: 追问：推动「客服系统的可观测性：消息丢失率 / 响应时延 / SLA 怎么监控」落地时，你会如何设计灰度、回滚和迁移路径
difficulty: 资深
...
如果面试官追问：推动「客服系统的可观测性：消息丢失率 / 响应时延 / SLA 怎么监控」落地时，你会如何设计灰度、回滚和迁移路径？
...
- 落地时先收敛改动面：选低风险页面或模块试点，保留旧路径作为对照。
- 迁移计划要说明数据兼容、配置开关、监控指标和失败后的恢复路径。
- 团队推进重点不是一次性重写，而是把「客服系统的可观测性：消息丢失率 / 响应时延 / SLA 怎么监控」拆成可验证的小步骤，逐步替换高风险部分。
