...
- 如果把「TCP、TLS、HTTP 三层关系怎么向面试官讲清楚」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「WebSocket、SSE、轮询怎么选」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「大文件上传、断点续传、Range 下载的前端设计」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「DNS、CDN 与接入层优化的前端视角」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「WebRTC 基础：为什么 P2P 仍然需要服务器」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「HTTP/3 / QUIC 在前端工程中的可见影响」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「HTTPS 握手过程，TLS 1.2 vs 1.3 有什么区别」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「长轮询 / WebSocket / SSE 怎么选」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「跨域与 CORS 预检，谁触发了 OPTIONS」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「HTTP 常见状态码及其含义」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「HTTP 103 Early Hints 是什么？怎么用来优化首屏」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「bfcache（前进/后退缓存）你怎么用好它」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
followups: [http1-vs-http2-multiplex-followup-1, http1-vs-http2-multiplex-followup-2, http1-vs-http2-multiplex-followup-3, http1-vs-http2-multiplex-followup-4, http1-vs-http2-multiplex-followup-5, http1-vs-http2-multiplex-followup-6, http1-vs-http2-multiplex-followup-7, http1-vs-http2-multiplex-followup-8, http1-vs-http2-multiplex-followup-9, http1-vs-http2-multiplex-followup-10, http1-vs-http2-multiplex-followup-11, http1-vs-http2-multiplex-followup-12, http1-vs-http2-multiplex-followup-13, http1-vs-http2-multiplex-followup-14]
difficulty: 进阶
tags: [HTTP/2, 多路复用, 高频]
...
title: 追问：如果把「TCP、TLS、HTTP 三层关系怎么向面试官讲清楚」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 基础
...
如果面试官追问：如果把「TCP、TLS、HTTP 三层关系怎么向面试官讲清楚」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：QUIC 的 0-RTT 是什么，有什么风险
difficulty: 进阶
tags: [HTTP2, HTTP3, QUIC, 追问]
parent: http1-http2-http3
...
如果面试官追问：QUIC 的 0-RTT 是什么，有什么风险（重放攻击）
...
title: 追问：immutable 这个 Cache-Control 指令做什么用
difficulty: 基础
tags: [缓存, CDN, 追问]
parent: caching
...
如果面试官追问：immutable 这个 Cache-Control 指令做什么用
...
title: 追问：预检的 Access-Control-Max-Age 一般设多久
difficulty: 进阶
tags: [跨域, CORS, 追问]
...
如果面试官追问：预检的 Access-Control-Max-Age 一般设多久
...
title: 追问：CORS 和 CSRF 谁防谁，能互相替代吗
difficulty: 进阶
tags: [跨域, CORS, 追问]
parent: cors-cross-origin
...
如果面试官追问：CORS 和 CSRF 谁防谁，能互相替代吗
...
title: 追问：跨域字体为什么要 crossorigin 属性
difficulty: 进阶
tags: [跨域, CORS, 追问]
parent: cors-cross-origin
...
如果面试官追问：跨域字体（@font-face）为什么要 crossorigin 属性
...
title: 追问：如果把「WebSocket、SSE、轮询怎么选」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [实时通信, SSE, 追问]
...
如果面试官追问：如果把「WebSocket、SSE、轮询怎么选」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「大文件上传、断点续传、Range 下载的前端设计」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [上传, Range, 追问]
...
如果面试官追问：如果把「大文件上传、断点续传、Range 下载的前端设计」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「DNS、CDN 与接入层优化的前端视角」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [DNS, CDN, 追问]
parent: dns-cdn
...
如果面试官追问：如果把「DNS、CDN 与接入层优化的前端视角」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「WebRTC 基础：为什么 P2P 仍然需要服务器」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [WebRTC, P2P, 追问]
parent: webrtc-basic
...
如果面试官追问：如果把「WebRTC 基础：为什么 P2P 仍然需要服务器」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「HTTP/3 / QUIC 在前端工程中的可见影响」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [HTTP/3, QUIC, 追问]
...
如果面试官追问：如果把「HTTP/3 / QUIC 在前端工程中的可见影响」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「HTTPS 握手过程，TLS 1.2 vs 1.3 有什么区别」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [TLS, 安全, 追问]
...
如果面试官追问：如果把「HTTPS 握手过程，TLS 1.2 vs 1.3 有什么区别」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「长轮询 / WebSocket / SSE 怎么选」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [实时, 推送, 追问]
...
如果面试官追问：如果把「长轮询 / WebSocket / SSE 怎么选」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「跨域与 CORS 预检，谁触发了 OPTIONS」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [CORS, 安全, 追问]
...
如果面试官追问：如果把「跨域与 CORS 预检，谁触发了 OPTIONS」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「HTTP 常见状态码及其含义」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 基础
tags: [HTTP, 高频, 追问]
parent: status-codes
...
如果面试官追问：如果把「HTTP 常见状态码及其含义」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「HTTP 103 Early Hints 是什么？怎么用来优化首屏」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [HTTP, 性能, 高频, 追问]
...
如果面试官追问：如果把「HTTP 103 Early Hints 是什么？怎么用来优化首屏」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「bfcache你怎么用好它」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [浏览器, 性能, 高频, 追问]
...
如果面试官追问：如果把「bfcache（前进/后退缓存）你怎么用好它」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：为什么 HTTP/2 还要 HPACK？普通 gzip 不行吗
difficulty: 进阶
...
如果面试官追问：为什么 HTTP/2 还要 HPACK？普通 gzip 不行吗？
...
- 头部：1.1 每次都明文重复（cookie 大头）；2 用 HPACK（静态表 + 动态表 + 哈夫曼）压缩
- TLS：HTTP/2 实际上强制 https（h2c over TCP 浏览器不实现）
- HTTP/2 后这些 hack 反而有害：分片破坏单连接复用，sprite 增加首包
- 可以补充一个真实项目语境：上线前先约定输入输出、失败兜底和观测指标，避免只在 demo 场景下成立。
...
title: 追问：HPACK 针对头部高重复结构设计，抗 CRIME/BREACH 攻击，gzip 头部用会有压力侧信道风险
difficulty: 进阶
tags: [HTTP/2, 多路复用, 高频, 追问]
parent: http1-vs-http2-multiplex
...
如果面试官追问：HPACK 针对头部高重复结构设计，**抗 CRIME/BREACH 攻击**，gzip 头部用会有压力侧信道风险
...
## http1-vs-http2-multiplex-followup-4

title: 追问：客户端缓存无法跨页面感知；推送命中率低；发出推送时连接已忙
difficulty: 进阶
tags: [HTTP/2, 多路复用, 高频, 追问]
parent: http1-vs-http2-multiplex
...
如果面试官追问：客户端缓存无法跨页面感知；推送命中率低（推了客户端早缓存的）；发出推送时连接已忙
...
## http1-vs-http2-multiplex-followup-5
...
## http1-vs-http2-multiplex-followup-6

title: 追问：HTTP/2 priority 树，现代浏览器用 priority 提示和 <link fetchpriority="high"
difficulty: 进阶
tags: [HTTP/2, 多路复用, 高频, 追问]
parent: http1-vs-http2-multiplex
...
如果面试官追问：HTTP/2 priority 树（已废弃为 RFC 9218 / Extensible Priorities），现代浏览器用 `priority` 提示和 `<link fetchpriority="high">`
...
## http1-vs-http2-multiplex-followup-7

title: 追问：HTTP/3 / QUIC 解决了什么 HTTP/2 的痛点
difficulty: 进阶
tags: [HTTP/2, 多路复用, 高频, 追问]
parent: http1-vs-http2-multiplex
...
如果面试官追问：HTTP/3 / QUIC 解决了什么 HTTP/2 的痛点？
...
## http1-vs-http2-multiplex-followup-8

title: 追问：TCP 队头阻塞：QUIC 在 UDP 上自实现 stream，丢包只影响一个 stream
difficulty: 进阶
tags: [HTTP/2, 多路复用, 高频, 追问]
parent: http1-vs-http2-multiplex
...
如果面试官追问：**TCP 队头阻塞**：QUIC 在 UDP 上自实现 stream，丢包只影响一个 stream
...
## http1-vs-http2-multiplex-followup-9

title: 追问：更快的握手：0-RTT / 1-RTT 重连
difficulty: 进阶
tags: [HTTP/2, 多路复用, 高频, 追问]
parent: http1-vs-http2-multiplex
...
如果面试官追问：**更快的握手**：0-RTT / 1-RTT 重连（带连接迁移）
...
## http1-vs-http2-multiplex-followup-10

title: 追问：连接迁移：手机 4G→WiFi 不断流
difficulty: 进阶
tags: [HTTP/2, 多路复用, 高频, 追问]
parent: http1-vs-http2-multiplex
...
如果面试官追问：**连接迁移**：手机 4G→WiFi 不断流（Connection ID 而非五元组）
...
## http1-vs-http2-multiplex-followup-11

title: 追问：大量 stream 并发会不会撑爆服务端
difficulty: 进阶
tags: [HTTP/2, 多路复用, 高频, 追问]
parent: http1-vs-http2-multiplex
...
## http1-vs-http2-multiplex-followup-12

title: 追问：SETTINGSMAXCONCURRENTSTREAMS 限制；超出会被 RSTSTREAM
difficulty: 进阶
tags: [HTTP/2, 多路复用, 高频, 追问]
parent: http1-vs-http2-multiplex
...
如果面试官追问：`SETTINGS_MAX_CONCURRENT_STREAMS` 限制（通常 128/256）；超出会被 RST_STREAM
...
## http1-vs-http2-multiplex-followup-13

title: 追问：HTTP/2 流式上传/下载的代码层面跟 1.1 有差异吗
difficulty: 进阶
tags: [HTTP/2, 多路复用, 高频, 追问]
parent: http1-vs-http2-multiplex
...
如果面试官追问：HTTP/2 流式上传/下载的代码层面跟 1.1 有差异吗？
...
## http1-vs-http2-multiplex-followup-14

title: 追问：应用层 fetch / WebSocket 几乎透明；Streams API + ReadableStream 可双向流，但浏览器对 fetch 上传 stream 支持滞后
difficulty: 进阶
tags: [HTTP/2, 多路复用, 高频, 追问]
parent: http1-vs-http2-multiplex
...
如果面试官追问：应用层 fetch / WebSocket 几乎透明；Streams API + ReadableStream 可双向流，但浏览器对 fetch 上传 stream 支持滞后（需要 HTTP/2 + isomorphic 配置）
