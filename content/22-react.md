...
- 实施步骤：围绕 Hooks 的核心规则与原理 用“前提 -> 机制 -> 失效场景”三段式解释，保证因果闭环，并且动作要交代渲染边界、状态分层和失效策略。
...
- 实施步骤：先把 TypeScript 拆成可执行子步骤，再逐步落地并记录每一步的输入输出，并且动作要交代渲染边界、状态分层和失效策略。
- 失败风险：常见风险是状态源混用，出现重复请求、脏读或 UI 闪烁。
- 验收信号：验收至少看渲染次数、请求重复率和状态一致性告警。
...
- 实施步骤：围绕 列表渲染中 key 的作用与使用陷阱 用“前提 -> 机制 -> 失效场景”三段式解释，保证因果闭环，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
...
回答这题时，先给 React 的结论，再给落地步骤，最后给可验证的验收标准。
...
- 追问核心：解释「React 和 Vue 的心智模型本质差异」背后的因果关系，并指出 React 的触发条件（对应追问：在「React 和 Vue 的心智模型本质差异」场景下，当「React 和 Vue 的心智模型本质差异」牵涉跨组件状态时，你会如何围绕 React 设计响应式边界，保证后续好维护）。
- 直接围绕「在「React 和 Vue 的心智模型本质差异」场景下，当「React 和 Vue 的心智模型本质差异」牵涉跨组件状态时，你会如何围绕 React 设计响应式边界，保证后续好维护」作答：响应式：Vue 基于依赖追踪（Proxy / getter），自动收集依赖；React 基于"状态变化触发整个子树重渲染 + 用 memo / hooks 控制"

#### 落地步骤

- 第一步：讨论 React 和 Vue 的心智模型本质差异 时要交代响应式依赖怎么收集、何时触发更新、如何清理副作用。
- 第二步：围绕 React 和 Vue 的心智模型本质差异 用“前提 -> 机制 -> 失效场景”三段式解释，保证因果闭环，并且动作要交代响应式依赖和组件更新时机，避免副作用漂移。
- 第三步：如果 React 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
回答这题时，先给 Hooks 的结论，再给落地步骤，最后给可验证的验收标准。
...
- 追问核心：围绕「Hooks 的核心规则与原理」给出可执行的落地方案，重点说明 Hooks 怎么做（对应追问：结合真实业务约束，在 React 项目里应用「Hooks 的核心规则与原理」时，哪些 state 或渲染边界最容易出问题）。
- 直接围绕「结合真实业务约束，在 React 项目里应用「Hooks 的核心规则与原理」时，哪些 state 或渲染边界最容易出问题」作答：Hooks 依靠"调用顺序"在内部链表里定位每个 hook 的状态槽，跳过会错位

#### 落地步骤

- 第一步：Hooks 的核心规则与原理 只有在瓶颈被数据证实时才值得推进；先确认 Hooks 是否真是主耗时来源。
- 第二步：先把 Hooks 拆成可执行子步骤，再逐步落地并记录每一步的输入输出，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 Hooks 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。

#### 风险与验收

- 失败风险：常见失败是只优化实验室分数，真实设备或弱网下 Hooks 的核心规则与原理 体验反而抖动。
- 验收信号：验收至少包含真实用户指标、分位耗时和错误率对照，而非单一跑分。
...
这道追问要直接回应「useEffect 常见陷阱与依赖管理」在 useEffect 上的执行动作、风险边界和验收信号。
...
- 追问核心：识别「useEffect 常见陷阱与依赖管理」的高风险失败场景并给出兜底措施（对应追问：在「useEffect 常见陷阱与依赖管理」进入长周期维护后，你会重点巡检哪些与 useEffect 相关的高风险边界点）。
- 直接围绕「在「useEffect 常见陷阱与依赖管理」进入长周期维护后，你会重点巡检哪些与 useEffect 相关的高风险边界点」作答：依赖数组不全：导致捕获旧值（stale closure），看似"没起效"

#### 落地步骤

- 第一步：先定义 useEffect 常见陷阱与依赖管理 的输入约束和成功标准，再展开实现细节，避免答案停在概念层。
- 第二步：先枚举高风险失败模式，再给降级和兜底动作，最后补恢复路径，并且动作要能被他人复现：步骤清晰、信号可观测、异常可回退。
- 第三步：如果 useEffect 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。

#### 风险与验收

- 失败风险：常见风险是只给理想路径，忽略 useEffect 的失败分支与恢复动作。
- 验收信号：验收至少包含围绕 useEffect 的可复现用例、线上监控指标和回退演练记录。
...
回答这题时，先给 Fiber 的结论，再给落地步骤，最后给可验证的验收标准。
...
- 追问核心：说明如何验证「Fiber 架构与并发渲染」结论成立，给出 Fiber 的验收路径（对应追问：从工程落地角度看，你会先看哪些与 Fiber 相关的指标来判断「Fiber 架构与并发渲染」是不是当前性能瓶颈）。
- 直接围绕「从工程落地角度看，你会先看哪些与 Fiber 相关的指标来判断「Fiber 架构与并发渲染」是不是当前性能瓶颈」作答：Fiber 是 React 16+ 的渲染单元，把组件树拆成可中断、可恢复的工作单元链表

#### 落地步骤

- 第一步：Fiber 架构与并发渲染 只有在瓶颈被数据证实时才值得推进；先确认 Fiber 是否真是主耗时来源。
- 第二步：把验证拆成“可复现样例 + 指标监控 + 告警阈值”，确保结论可复核，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 Fiber 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
- 追问核心：围绕「React Server Components」给出可执行的落地方案，重点说明 React 怎么做（对应追问：在 React 项目里应用「React Server Components」时，哪些 state 或渲染边界最容易出问题）。
- 直接围绕「在 React 项目里应用「React Server Components」时，哪些 state 或渲染边界最容易出问题」作答：SSR：服务端渲出 HTML 再 hydration，组件代码仍要打到客户端 bundle

#### 落地步骤

- 第一步：React Server Components 只有在瓶颈被数据证实时才值得推进；先确认 React 是否真是主耗时来源。
- 第二步：先把 React 拆成可执行子步骤，再逐步落地并记录每一步的输入输出，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 React 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。

#### 风险与验收

- 失败风险：常见失败是只优化实验室分数，真实设备或弱网下 React Server Components 体验反而抖动。
- 验收信号：验收至少包含真实用户指标、分位耗时和错误率对照，而非单一跑分。
...
这道追问要直接回应「React Server Components」在 从工程落地角度看 上的执行动作、风险边界和验收信号。
...
- 追问核心：说明如何验证「React Server Components」结论成立，给出 从工程落地角度看 的验收路径（对应追问：从工程落地角度看，上线后你会盯哪些与 RSC 相关的日志与指标，来确认这套方案确实带来改进）。
- 直接围绕「从工程落地角度看，上线后你会盯哪些与 RSC 相关的日志与指标，来确认这套方案确实带来改进」作答：SSR：服务端渲出 HTML 再 hydration，组件代码仍要打到客户端 bundle

#### 落地步骤

- 第一步：React Server Components 的前置条件是迁移批次、灰度门槛和回滚策略明确，不然执行风险不可控。
- 第二步：先定义验收指标，再安排离线用例和线上观测，最后给失败阈值与回滚条件，并且动作要同时交代迁移批次、灰度策略和回滚门槛。
- 第三步：如果 从工程落地角度看 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
这道追问的关键是把 别只看 讲成可执行方案，并补风险与验收闭环。
...
- 追问核心：比较「React Server Components」在收益、成本和维护复杂度上的取舍边界（对应追问：别只看 benchmark，你会怎么比较「React Server Components」和替代方案在 RSC 上的学习成本、交付速度与维护负担）。
- 直接围绕「别只看 benchmark，你会怎么比较「React Server Components」和替代方案在 RSC 上的学习成本、交付速度与维护负担」作答：SSR：服务端渲出 HTML 再 hydration，组件代码仍要打到客户端 bundle

#### 落地步骤

- 第一步：回答 React Server Components 时要说明 别只看 在并发渲染下的行为差异和回归策略。
- 第二步：对比候选方案时同时给短期交付风险和长期治理成本，再做最终取舍，并且动作要交代渲染边界、状态分层和失效策略。
- 第三步：如果 别只看 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
- 实施步骤：先把 React 拆成可执行子步骤，再逐步落地并记录每一步的输入输出，并且动作要交代渲染边界、状态分层和失效策略。
- 失败风险：常见风险是状态源混用，出现重复请求、脏读或 UI 闪烁。
- 验收信号：验收至少看渲染次数、请求重复率和状态一致性告警。
...
回答这题时，先给 在当前团队与业务约束 的结论，再给落地步骤，最后给可验证的验收标准。
...
- 追问核心：围绕「React 状态管理选型」给出可执行的落地方案，重点说明 在当前团队与业务约束 怎么做（对应追问：在当前团队与业务约束下，在 React 项目里应用「React 状态管理选型」时，哪些 state 或渲染边界最容易出问题）。
- 直接围绕「在当前团队与业务约束下，在 React 项目里应用「React 状态管理选型」时，哪些 state 或渲染边界最容易出问题」作答：Context：只适合"低频变更 + 全局静态值"（主题、locale），频繁变更会让所有 consumer 重渲染

#### 落地步骤

- 第一步：React 状态管理选型 只有在瓶颈被数据证实时才值得推进；先确认 在当前团队与业务约束 是否真是主耗时来源。
- 第二步：先把 在当前团队与业务约束 拆成可执行子步骤，再逐步落地并记录每一步的输入输出，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 在当前团队与业务约束 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。

#### 风险与验收

- 失败风险：常见失败是只优化实验室分数，真实设备或弱网下 React 状态管理选型 体验反而抖动。
- 验收信号：验收至少包含真实用户指标、分位耗时和错误率对照，而非单一跑分。
...
这道追问的关键是把 Suspense 讲成可执行方案，并补风险与验收闭环。
...
- 追问核心：识别「Suspense 与异步数据加载」的高风险失败场景并给出兜底措施（对应追问：在「Suspense 与异步数据加载」进入长周期维护后，你会重点巡检哪些与 Suspense 相关的高风险边界点）。
- 直接围绕「在「Suspense 与异步数据加载」进入长周期维护后，你会重点巡检哪些与 Suspense 相关的高风险边界点」作答：Suspense 让组件在数据未就绪时"挂起"，由父级 fallback 渲染 spinner，无需手写 isLoading 分支

#### 落地步骤

- 第一步：回答 Suspense 与异步数据加载 时先锁定 Suspense 的边界条件，避免把经验结论当成通用规则。
- 第二步：围绕 Suspense 的故障点做演练，明确“发现问题 -> 降级 -> 恢复”链路，并且动作需要包含完成标准和失败处理，不要只给方向不写执行细节。
- 第三步：如果 Suspense 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
这道追问要直接回应「React Router v6.4+ 的 Data Router 与 loaders」在 Router 上的执行动作、风险边界和验收信号。
...
- 追问核心：围绕「React Router v6.4+ 的 Data Router 与 loaders」给出可执行的落地方案，重点说明 Router 怎么做（对应追问：在「React Router v6.4+ 的 Data Router 与 loaders」场景下，在 React 项目里应用「React Router v6.4+ 的 Data Router 与 loaders」时，哪些 state 或渲染边界最容易出问题）。
- 直接围绕「在「React Router v6.4+ 的 Data Router 与 loaders」场景下，在 React 项目里应用「React Router v6.4+ 的 Data Router 与 loaders」时，哪些 state 或渲染边界最容易出问题」作答：路由级 loader 在切换前并行发起请求，配合 defer / Await 可以做"先关键内容 + 流式补齐"

#### 落地步骤

- 第一步：回答 React Router v6.4+ 的 Data Router 与 loaders 前先给基线：至少明确耗时分位、错误率和用户体感，不然优化动作没有参照。
- 第二步：按“先复现现状 -> 再最小改动实现 -> 最后补回归”推进，确保每一步都可回退，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 Router 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。

#### 风险与验收

- 失败风险：常见失败是只优化实验室分数，真实设备或弱网下 React Router v6.4+ 的 Data Router 与 loaders 体验反而抖动。
- 验收信号：验收至少包含真实用户指标、分位耗时和错误率对照，而非单一跑分。
...
围绕「React 性能优化清单」回答追问时，重点说清 从工程落地角度看 的前提、动作和回退条件。
...
- 追问核心：说明如何验证「React 性能优化清单」结论成立，给出 从工程落地角度看 的验收路径（对应追问：从工程落地角度看，你会先看哪些与 性能 相关的指标来判断「React 性能优化清单」是不是当前性能瓶颈）。
- 直接围绕「从工程落地角度看，你会先看哪些与 性能 相关的指标来判断「React 性能优化清单」是不是当前性能瓶颈」作答：首先用 React DevTools Profiler 录一段，看哪个组件 commit 时间长、commit 次数多

#### 落地步骤

- 第一步：回答 React 性能优化清单 前先给基线：至少明确耗时分位、错误率和用户体感，不然优化动作没有参照。
...
围绕「React 19 关键特性速览」回答追问时，重点说清 React 19 的前提、动作和回退条件。
...
- 追问核心：围绕「React 19 关键特性速览」给出可执行的落地方案，重点说明 React 19 怎么做（对应追问：在 React 项目里应用「React 19 关键特性速览」时，哪些 state 或渲染边界最容易出问题）。
- 直接围绕「在 React 项目里应用「React 19 关键特性速览」时，哪些 state 或渲染边界最容易出问题」作答：Actions：把"提交 + pending + error + revalidate"封装成约定，配合 form / button

#### 落地步骤

- 第一步：回答 React 19 关键特性速览 前先给基线：至少明确耗时分位、错误率和用户体感，不然优化动作没有参照。
- 第二步：按“先复现现状 -> 再最小改动实现 -> 最后补回归”推进，确保每一步都可回退，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 React 19 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。

#### 风险与验收

- 失败风险：常见失败是只优化实验室分数，真实设备或弱网下 React 19 关键特性速览 体验反而抖动。
- 验收信号：验收至少包含真实用户指标、分位耗时和错误率对照，而非单一跑分。
...
这道追问的关键是把 TypeScript 讲成可执行方案，并补风险与验收闭环。
...
- 追问核心：围绕「React + TypeScript 常用类型模式」给出可执行的落地方案，重点说明 TypeScript 怎么做（对应追问：在当前团队与业务约束下，在 React 项目里应用「React + TypeScript 常用类型模式」时，哪些 state 或渲染边界最容易出问题）。
- 直接围绕「在当前团队与业务约束下，在 React 项目里应用「React + TypeScript 常用类型模式」时，哪些 state 或渲染边界最容易出问题」作答：as prop（多态组件）：让组件可以渲染成不同标签 / 组件

#### 落地步骤

- 第一步：React + TypeScript 常用类型模式 只有在瓶颈被数据证实时才值得推进；先确认 TypeScript 是否真是主耗时来源。
- 第二步：先把 TypeScript 拆成可执行子步骤，再逐步落地并记录每一步的输入输出，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 TypeScript 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。

#### 风险与验收

- 失败风险：常见失败是只优化实验室分数，真实设备或弱网下 React + TypeScript 常用类型模式 体验反而抖动。
- 验收信号：验收至少包含真实用户指标、分位耗时和错误率对照，而非单一跑分。
...
- 追问核心：说明如何验证「React 组件测试要测什么、怎么测」结论成立，给出 测试 的验收路径（对应追问：在 React 项目里应用「React 组件测试要测什么、怎么测」时，哪些 state 或渲染边界最容易出问题）。
- 直接围绕「在 React 项目里应用「React 组件测试要测什么、怎么测」时，哪些 state 或渲染边界最容易出问题」作答："像用户一样测试"：不要测实现细节（state、私有方法），要测可见行为

#### 落地步骤

- 第一步：React 组件测试要测什么、怎么测 只有在瓶颈被数据证实时才值得推进；先确认 测试 是否真是主耗时来源。
- 第二步：把验证拆成“可复现样例 + 指标监控 + 告警阈值”，确保结论可复核，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 测试 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。

#### 风险与验收

- 失败风险：常见失败是只优化实验室分数，真实设备或弱网下 React 组件测试要测什么、怎么测 体验反而抖动。
- 验收信号：验收至少包含真实用户指标、分位耗时和错误率对照，而非单一跑分。
...
这道追问要直接回应「列表渲染中 key 的作用与使用陷阱」在 Diff 上的执行动作、风险边界和验收信号。
...
- 追问核心：说明如何验证「列表渲染中 key 的作用与使用陷阱」结论成立，给出 Diff 的验收路径（对应追问：从工程落地角度看，你会先看哪些与 Diff 相关的指标来判断「列表渲染中 key 的作用与使用陷阱」是不是当前性能瓶颈）。
- 直接围绕「从工程落地角度看，你会先看哪些与 Diff 相关的指标来判断「列表渲染中 key 的作用与使用陷阱」是不是当前性能瓶颈」作答：key 帮助 React 在 reconcile 时识别哪些元素是「同一个」，决定复用、移动还是销毁

#### 落地步骤

- 第一步：回答 列表渲染中 key 的作用与使用陷阱 前先给基线：至少明确耗时分位、错误率和用户体感，不然优化动作没有参照。
- 第二步：先定义验收指标，再安排离线用例和线上观测，最后给失败阈值与回滚条件，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
...
这道追问的关键是把 表单 讲成可执行方案，并补风险与验收闭环。
...
- 追问核心：说明如何验证「受控组件 vs 非受控组件，性能边界在哪」结论成立，给出 表单 的验收路径（对应追问：排查「受控组件 vs 非受控组件，性能边界在哪」性能瓶颈时，你会先看哪些指标来判断它是不是主矛盾）。
- 直接围绕「排查「受控组件 vs 非受控组件，性能边界在哪」性能瓶颈时，你会先看哪些指标来判断它是不是主矛盾」作答：受控：value + onChange，组件自身不存储状态，便于校验/联动

#### 落地步骤

- 第一步：受控组件 vs 非受控组件，性能边界在哪 只有在瓶颈被数据证实时才值得推进；先确认 表单 是否真是主耗时来源。
- 第二步：把验证拆成“可复现样例 + 指标监控 + 告警阈值”，确保结论可复核，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
...
围绕「Portal、Error Boundary、Suspense 的协作方式」回答追问时，重点说清 架构 的前提、动作和回退条件。
...
- 追问核心：说明「Portal、Error Boundary、Suspense 的协作方式」上线时如何灰度、观测、回滚（对应追问：真要把「Portal、Error Boundary、Suspense 的协作方式」推到线上，你会如何围绕 架构 设计灰度节奏、回滚条件和迁移路径）。
- 直接围绕「真要把「Portal、Error Boundary、Suspense 的协作方式」推到线上，你会如何围绕 架构 设计灰度节奏、回滚条件和迁移路径」作答：Portal：createPortal(children, document.body)，把子树渲染到任意 DOM，但事件冒泡仍按 React 树

#### 落地步骤

- 第一步：Portal、Error Boundary、Suspense 的协作方式 的前置条件是迁移批次、灰度门槛和回滚策略明确，不然执行风险不可控。
...
围绕「React 列表为什么必须给 key」回答追问时，重点说清 list 的前提、动作和回退条件。
...
- 追问核心：解释「React 列表为什么必须给 key」背后的因果关系，并指出 list 的触发条件（对应追问：结合真实业务约束，在 React 项目里应用「React 列表为什么必须给 key」时，哪些 state 或渲染边界最容易出问题）。
- 直接围绕「结合真实业务约束，在 React 项目里应用「React 列表为什么必须给 key」时，哪些 state 或渲染边界最容易出问题」作答：diff 算法靠 key 在同层之间做"身份匹配"。没 key 就只能按位置匹配，插入/删除前面的元素会让后面所有节点的状态错位

#### 落地步骤

- 第一步：回答 React 列表为什么必须给 key 前先给基线：至少明确耗时分位、错误率和用户体感，不然优化动作没有参照。
- 第二步：先说触发条件，再解释机制，再给反例，避免把“结果”当成“原因”，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 list 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。

#### 风险与验收

- 失败风险：常见失败是只优化实验室分数，真实设备或弱网下 React 列表为什么必须给 key 体验反而抖动。
- 验收信号：验收至少包含真实用户指标、分位耗时和错误率对照，而非单一跑分。
...
回答这题时，先给 list 的结论，再给落地步骤，最后给可验证的验收标准。
...
- 追问核心：解释「React 列表为什么必须给 key」背后的因果关系，并指出 list 的触发条件（对应追问：在「React 列表为什么必须给 key」场景下，为了证明这个方案在 list 维度有效，你会怎么设计测试闭环和线上观测指标）。
- 直接围绕「在「React 列表为什么必须给 key」场景下，为了证明这个方案在 list 维度有效，你会怎么设计测试闭环和线上观测指标」作答：diff 算法靠 key 在同层之间做"身份匹配"。没 key 就只能按位置匹配，插入/删除前面的元素会让后面所有节点的状态错位

#### 落地步骤

- 第一步：React 列表为什么必须给 key 的测试价值来自“可复核”；先约定如何在 CI 与线上同时验证 list。
- 第二步：围绕 React 列表为什么必须给 key 用“前提 -> 机制 -> 失效场景”三段式解释，保证因果闭环，并且动作要覆盖边界输入、时序异常和回归用例，避免“只测主路径”。
- 第三步：如果 list 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
- 追问核心：解释「React 列表为什么必须给 key」背后的因果关系，并指出 list 的触发条件（对应追问：如果团队能力结构变化，你会怎样围绕 list 调整「React 列表为什么必须给 key」与替代方案的选型结论）。
- 直接围绕「如果团队能力结构变化，你会怎样围绕 list 调整「React 列表为什么必须给 key」与替代方案的选型结论」作答：diff 算法靠 key 在同层之间做"身份匹配"。没 key 就只能按位置匹配，插入/删除前面的元素会让后面所有节点的状态错位

#### 落地步骤

- 第一步：回答 React 列表为什么必须给 key 时要说明 list 在并发渲染下的行为差异和回归策略。
- 第二步：围绕 React 列表为什么必须给 key 用“前提 -> 机制 -> 失效场景”三段式解释，保证因果闭环，并且动作要交代渲染边界、状态分层和失效策略。
- 第三步：如果 list 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
这道追问的关键是把 React 讲成可执行方案，并补风险与验收闭环。
...
- 追问核心：解释「React 和 Vue 的心智模型本质差异」背后的因果关系，并指出 React 的触发条件（对应追问：在当前团队与业务约束下，你会如何把「React 和 Vue 的心智模型本质差异」拆成可观测、可回滚的小单元，避免一次性大改）。
- 直接围绕「在当前团队与业务约束下，你会如何把「React 和 Vue 的心智模型本质差异」拆成可观测、可回滚的小单元，避免一次性大改」作答：响应式：Vue 基于依赖追踪（Proxy / getter），自动收集依赖；React 基于"状态变化触发整个子树重渲染 + 用 memo / hooks 控制"

#### 落地步骤

- 第一步：落地 React 和 Vue 的心智模型本质差异 前要先确认团队协作链路：谁发布、谁观测、谁回退，避免责任空窗。
- 第二步：围绕 React 和 Vue 的心智模型本质差异 用“前提 -> 机制 -> 失效场景”三段式解释，保证因果闭环，并且动作要同时交代迁移批次、灰度策略和回滚门槛。
- 第三步：如果 React 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
- 追问核心：解释「React 和 Vue 的心智模型本质差异」背后的因果关系，并指出 React 的触发条件（对应追问：结合真实业务约束，在评审「React 和 Vue 的心智模型本质差异」时，你会如何围绕 React 向团队解释“什么时候值得用，什么时候别硬上”）。
- 直接围绕「结合真实业务约束，在评审「React 和 Vue 的心智模型本质差异」时，你会如何围绕 React 向团队解释“什么时候值得用，什么时候别硬上”」作答：响应式：Vue 基于依赖追踪（Proxy / getter），自动收集依赖；React 基于"状态变化触发整个子树重渲染 + 用 memo / hooks 控制"
...
这道追问要直接回应「Hooks 的核心规则与原理」在 Hooks 上的执行动作、风险边界和验收信号。
...
- 追问核心：说明如何验证「Hooks 的核心规则与原理」结论成立，给出 Hooks 的验收路径（对应追问：在「Hooks 的核心规则与原理」场景下，为了证明这个方案在 Hooks 维度有效，你会怎么设计测试闭环和线上观测指标）。
- 直接围绕「在「Hooks 的核心规则与原理」场景下，为了证明这个方案在 Hooks 维度有效，你会怎么设计测试闭环和线上观测指标」作答：Hooks 依靠"调用顺序"在内部链表里定位每个 hook 的状态槽，跳过会错位

#### 落地步骤

- 第一步：回答 Hooks 的核心规则与原理 时先定义验收口径：主路径、边界输入和失败分支都要有可复现用例。
...
- 追问核心：围绕「Hooks 的核心规则与原理」给出可执行的落地方案，重点说明 Hooks 怎么做（对应追问：以「Hooks 的核心规则与原理」为例，如果团队人数、交付节奏和业务复杂度不同，你会怎么基于 Hooks 判断该不该选「Hooks 的核心规则与原理」）。
- 直接围绕「以「Hooks 的核心规则与原理」为例，如果团队人数、交付节奏和业务复杂度不同，你会怎么基于 Hooks 判断该不该选「Hooks 的核心规则与原理」」作答：Hooks 依靠"调用顺序"在内部链表里定位每个 hook 的状态槽，跳过会错位

#### 落地步骤

- 第一步：先声明输入规模和内存预算，再讨论 Hooks 的核心规则与原理；复杂度边界不清会导致方案失真。
...
围绕「useEffect 常见陷阱与依赖管理」回答追问时，重点说清 useEffect 的前提、动作和回退条件。
...
- 追问核心：围绕「useEffect 常见陷阱与依赖管理」给出可执行的落地方案，重点说明 useEffect 怎么做（对应追问：从工程落地角度看，React 18 自动批处理对 effect 的影响）。
- 直接围绕「从工程落地角度看，React 18 自动批处理对 effect 的影响」作答：依赖数组不全：导致捕获旧值（stale closure），看似"没起效"

#### 落地步骤

- 第一步：useEffect 常见陷阱与依赖管理 的前置条件是迁移批次、灰度门槛和回滚策略明确，不然执行风险不可控。
...
- 追问核心：围绕「useEffect 常见陷阱与依赖管理」给出可执行的落地方案，重点说明 useEffect 怎么做（对应追问：useRef + useEffect 模拟「上一次值」的写法）。
- 直接围绕「useRef + useEffect 模拟「上一次值」的写法」作答：依赖数组不全：导致捕获旧值（stale closure），看似"没起效"
...
这道追问要直接回应「Fiber 架构与并发渲染」在 Fiber 上的执行动作、风险边界和验收信号。
...
- 追问核心：说明如何验证「Fiber 架构与并发渲染」结论成立，给出 Fiber 的验收路径（对应追问：围绕「Fiber 架构与并发渲染」上线效果，你会优先看哪些和 Fiber 相关的真实用户指标来佐证体验提升）。
- 直接围绕「围绕「Fiber 架构与并发渲染」上线效果，你会优先看哪些和 Fiber 相关的真实用户指标来佐证体验提升」作答：Fiber 是 React 16+ 的渲染单元，把组件树拆成可中断、可恢复的工作单元链表

#### 落地步骤

- 第一步：回答 Fiber 架构与并发渲染 前先给基线：至少明确耗时分位、错误率和用户体感，不然优化动作没有参照。
- 第二步：先定义验收指标，再安排离线用例和线上观测，最后给失败阈值与回滚条件，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 Fiber 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
这道追问的关键是把 Fiber 讲成可执行方案，并补风险与验收闭环。
...
- 追问核心：比较「Fiber 架构与并发渲染」在收益、成本和维护复杂度上的取舍边界（对应追问：以「Fiber 架构与并发渲染」为例，如果「Fiber 架构与并发渲染」在 Fiber 上的收益和维护成本打架，你会怎么做取舍判断）。
- 直接围绕「以「Fiber 架构与并发渲染」为例，如果「Fiber 架构与并发渲染」在 Fiber 上的收益和维护成本打架，你会怎么做取舍判断」作答：Fiber 是 React 16+ 的渲染单元，把组件树拆成可中断、可恢复的工作单元链表
...
回答这题时，先给 状态管理 的结论，再给落地步骤，最后给可验证的验收标准。
...
- 追问核心：说明如何验证「React 状态管理选型」结论成立，给出 状态管理 的验收路径（对应追问：结合真实业务约束，你会如何围绕 状态管理 定义“方案生效”的判据，并通过测试与观测数据持续验证）。
- 直接围绕「结合真实业务约束，你会如何围绕 状态管理 定义“方案生效”的判据，并通过测试与观测数据持续验证」作答：Context：只适合"低频变更 + 全局静态值"（主题、locale），频繁变更会让所有 consumer 重渲染

#### 落地步骤

- 第一步：React 状态管理选型 的测试价值来自“可复核”；先约定如何在 CI 与线上同时验证 状态管理。
...
- 追问核心：围绕「React 状态管理选型」给出可执行的落地方案，重点说明 状态管理 怎么做（对应追问：在当前团队与业务约束下，围绕「React 状态管理选型」选型时，你会怎样按 状态管理 与业务复杂度给出分层推荐）。
- 直接围绕「在当前团队与业务约束下，围绕「React 状态管理选型」选型时，你会怎样按 状态管理 与业务复杂度给出分层推荐」作答：Context：只适合"低频变更 + 全局静态值"（主题、locale），频繁变更会让所有 consumer 重渲染

#### 落地步骤

- 第一步：回答 React 状态管理选型 时要说明 状态管理 在极端输入下的行为，不要只给样例路径。
- 第二步：先把 状态管理 拆成可执行子步骤，再逐步落地并记录每一步的输入输出，并且动作要附上复杂度分析与边界样例，证明在规模变化时仍可用。
- 第三步：如果 状态管理 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
这道追问的关键是把 Router 讲成可执行方案，并补风险与验收闭环。
...
- 追问核心：说明如何验证「React Router v6.4+ 的 Data Router 与 loaders」结论成立，给出 Router 的验收路径（对应追问：结合真实业务约束，你会如何围绕 Router 定义“方案生效”的判据，并通过测试与观测数据持续验证）。
- 直接围绕「结合真实业务约束，你会如何围绕 Router 定义“方案生效”的判据，并通过测试与观测数据持续验证」作答：路由级 loader 在切换前并行发起请求，配合 defer / Await 可以做"先关键内容 + 流式补齐"

#### 落地步骤

- 第一步：React Router v6.4+ 的 Data Router 与 loaders 的测试价值来自“可复核”；先约定如何在 CI 与线上同时验证 Router。
- 第二步：把验证拆成“可复现样例 + 指标监控 + 告警阈值”，确保结论可复核，并且动作要覆盖边界输入、时序异常和回归用例，避免“只测主路径”。
- 第三步：如果 Router 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
回答这题时，先给 Router 的结论，再给落地步骤，最后给可验证的验收标准。
...
- 追问核心：围绕「React Router v6.4+ 的 Data Router 与 loaders」给出可执行的落地方案，重点说明 Router 怎么做（对应追问：在「React Router v6.4+ 的 Data Router 与 loaders」场景下，围绕「React Router v6.4+ 的 Data Router 与 loaders」选型时，你会怎样按 Router 与业务复杂度给出分层推荐）。
- 直接围绕「在「React Router v6.4+ 的 Data Router 与 loaders」场景下，围绕「React Router v6.4+ 的 Data Router 与 loaders」选型时，你会怎样按 Router 与业务复杂度给出分层推荐」作答：路由级 loader 在切换前并行发起请求，配合 defer / Await 可以做"先关键内容 + 流式补齐"

#### 落地步骤

- 第一步：回答 React Router v6.4+ 的 Data Router 与 loaders 时要说明 Router 在极端输入下的行为，不要只给样例路径。
- 第二步：先把 Router 拆成可执行子步骤，再逐步落地并记录每一步的输入输出，并且动作要附上复杂度分析与边界样例，证明在规模变化时仍可用。
- 第三步：如果 Router 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
- 追问核心：说明如何验证「React 性能优化清单」结论成立，给出 性能 的验收路径（对应追问：以「React 性能优化清单」为例，要证明「React 性能优化清单」确实改善体验，你会如何围绕 性能 设计线上观测与对照验证）。
- 直接围绕「以「React 性能优化清单」为例，要证明「React 性能优化清单」确实改善体验，你会如何围绕 性能 设计线上观测与对照验证」作答：首先用 React DevTools Profiler 录一段，看哪个组件 commit 时间长、commit 次数多

#### 落地步骤

- 第一步：React 性能优化清单 只有在瓶颈被数据证实时才值得推进；先确认 性能 是否真是主耗时来源。
- 第二步：把验证拆成“可复现样例 + 指标监控 + 告警阈值”，确保结论可复核，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
...
这道追问要直接回应「React 性能优化清单」在 性能 上的执行动作、风险边界和验收信号。
...
- 追问核心：比较「React 性能优化清单」在收益、成本和维护复杂度上的取舍边界（对应追问：结合真实业务约束，围绕「React 性能优化清单」在 性能 上的优化决策，你会如何量化收益、风险和长期维护成本）。
- 直接围绕「结合真实业务约束，围绕「React 性能优化清单」在 性能 上的优化决策，你会如何量化收益、风险和长期维护成本」作答：首先用 React DevTools Profiler 录一段，看哪个组件 commit 时间长、commit 次数多
...
这道追问的关键是把 React 19 讲成可执行方案，并补风险与验收闭环。
...
- 追问核心：说明如何验证「React 19 关键特性速览」结论成立，给出 React 19 的验收路径（对应追问：在「React 19 关键特性速览」场景下，为了证明这个方案在 React 19 维度有效，你会怎么设计测试闭环和线上观测指标）。
- 直接围绕「在「React 19 关键特性速览」场景下，为了证明这个方案在 React 19 维度有效，你会怎么设计测试闭环和线上观测指标」作答：Actions：把"提交 + pending + error + revalidate"封装成约定，配合 form / button

#### 落地步骤

- 第一步：React 19 关键特性速览 的测试价值来自“可复核”；先约定如何在 CI 与线上同时验证 React 19。
- 第二步：把验证拆成“可复现样例 + 指标监控 + 告警阈值”，确保结论可复核，并且动作要覆盖边界输入、时序异常和回归用例，避免“只测主路径”。
- 第三步：如果 React 19 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
回答这题时，先给 React 19 的结论，再给落地步骤，最后给可验证的验收标准。
...
- 追问核心：围绕「React 19 关键特性速览」给出可执行的落地方案，重点说明 React 19 怎么做（对应追问：在评审「React 19 关键特性速览」时，你会如何围绕 React 19 向团队解释“什么时候值得用，什么时候别硬上”）。
- 直接围绕「在评审「React 19 关键特性速览」时，你会如何围绕 React 19 向团队解释“什么时候值得用，什么时候别硬上”」作答：Actions：把"提交 + pending + error + revalidate"封装成约定，配合 form / button

#### 落地步骤

- 第一步：回答 React 19 关键特性速览 时要说明 React 19 在并发渲染下的行为差异和回归策略。
- 第二步：先把 React 19 拆成可执行子步骤，再逐步落地并记录每一步的输入输出，并且动作要交代渲染边界、状态分层和失效策略。
- 第三步：如果 React 19 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
围绕「React + TypeScript 常用类型模式」回答追问时，重点说清 TypeScript 的前提、动作和回退条件。
...
- 追问核心：说明如何验证「React + TypeScript 常用类型模式」结论成立，给出 TypeScript 的验收路径（对应追问：结合真实业务约束，如果要让结论在 TypeScript 上可复核，你会怎样安排测试、日志和指标的组合验证）。
- 直接围绕「结合真实业务约束，如果要让结论在 TypeScript 上可复核，你会怎样安排测试、日志和指标的组合验证」作答：as prop（多态组件）：让组件可以渲染成不同标签 / 组件

#### 落地步骤

- 第一步：回答 React + TypeScript 常用类型模式 时先定义验收口径：主路径、边界输入和失败分支都要有可复现用例。
- 第二步：先定义验收指标，再安排离线用例和线上观测，最后给失败阈值与回滚条件，并且动作要覆盖边界输入、时序异常和回归用例，避免“只测主路径”。
- 第三步：如果 TypeScript 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
这道追问要直接回应「React + TypeScript 常用类型模式」在 TypeScript 上的执行动作、风险边界和验收信号。
...
- 追问核心：围绕「React + TypeScript 常用类型模式」给出可执行的落地方案，重点说明 TypeScript 怎么做（对应追问：从工程落地角度看，和常见替代方案相比，「React + TypeScript 常用类型模式」在 TypeScript 这个维度更适合什么团队规模与业务复杂度）。
- 直接围绕「从工程落地角度看，和常见替代方案相比，「React + TypeScript 常用类型模式」在 TypeScript 这个维度更适合什么团队规模与业务复杂度」作答：as prop（多态组件）：让组件可以渲染成不同标签 / 组件

#### 落地步骤

- 第一步：React + TypeScript 常用类型模式 的前置条件是迁移批次、灰度门槛和回滚策略明确，不然执行风险不可控。
- 第二步：按“先复现现状 -> 再最小改动实现 -> 最后补回归”推进，确保每一步都可回退，并且动作要同时交代迁移批次、灰度策略和回滚门槛。
- 第三步：如果 TypeScript 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
围绕「React 组件测试要测什么、怎么测」回答追问时，重点说清 测试 的前提、动作和回退条件。
...
- 追问核心：说明如何验证「React 组件测试要测什么、怎么测」结论成立，给出 测试 的验收路径（对应追问：从工程落地角度看，上线后你会盯哪些与 测试链路 相关的日志与指标，来确认这套方案确实带来改进）。
- 直接围绕「从工程落地角度看，上线后你会盯哪些与 测试链路 相关的日志与指标，来确认这套方案确实带来改进」作答："像用户一样测试"：不要测实现细节（state、私有方法），要测可见行为

#### 落地步骤

- 第一步：回答 React 组件测试要测什么、怎么测 时先定义验收口径：主路径、边界输入和失败分支都要有可复现用例。
- 第二步：先定义验收指标，再安排离线用例和线上观测，最后给失败阈值与回滚条件，并且动作要覆盖边界输入、时序异常和回归用例，避免“只测主路径”。
- 第三步：如果 测试 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
- 追问核心：说明如何验证「React 组件测试要测什么、怎么测」结论成立，给出 测试 的验收路径（对应追问：以「React 组件测试要测什么、怎么测」为例，当业务复杂度升级时，你会如何判断「React 组件测试要测什么、怎么测」在 测试链路 上还能不能继续扛住）。
- 直接围绕「以「React 组件测试要测什么、怎么测」为例，当业务复杂度升级时，你会如何判断「React 组件测试要测什么、怎么测」在 测试链路 上还能不能继续扛住」作答："像用户一样测试"：不要测实现细节（state、私有方法），要测可见行为

#### 落地步骤

- 第一步：React 组件测试要测什么、怎么测 的测试价值来自“可复核”；先约定如何在 CI 与线上同时验证 测试。
- 第二步：把验证拆成“可复现样例 + 指标监控 + 告警阈值”，确保结论可复核，并且动作要覆盖边界输入、时序异常和回归用例，避免“只测主路径”。
- 第三步：如果 测试 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
回答这题时，先给 Diff 的结论，再给落地步骤，最后给可验证的验收标准。
...
- 追问核心：围绕「列表渲染中 key 的作用与使用陷阱」给出可执行的落地方案，重点说明 Diff 怎么做（对应追问：在当前团队与业务约束下，如果实验室分数变好但线上反馈一般，你会如何围绕 Diff 重新评估「列表渲染中 key 的作用与使用陷阱」优化效果）。
- 直接围绕「在当前团队与业务约束下，如果实验室分数变好但线上反馈一般，你会如何围绕 Diff 重新评估「列表渲染中 key 的作用与使用陷阱」优化效果」作答：key 帮助 React 在 reconcile 时识别哪些元素是「同一个」，决定复用、移动还是销毁

#### 落地步骤

- 第一步：列表渲染中 key 的作用与使用陷阱 只有在瓶颈被数据证实时才值得推进；先确认 Diff 是否真是主耗时来源。
- 第二步：先把 Diff 拆成可执行子步骤，再逐步落地并记录每一步的输入输出，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 Diff 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
围绕「列表渲染中 key 的作用与使用陷阱」回答追问时，重点说清 Diff 的前提、动作和回退条件。
...
- 追问核心：比较「列表渲染中 key 的作用与使用陷阱」在收益、成本和维护复杂度上的取舍边界（对应追问：在「列表渲染中 key 的作用与使用陷阱」场景下，你会怎样评估「列表渲染中 key 的作用与使用陷阱」在性能收益与兼容性风险之间的平衡点）。
- 直接围绕「在「列表渲染中 key 的作用与使用陷阱」场景下，你会怎样评估「列表渲染中 key 的作用与使用陷阱」在性能收益与兼容性风险之间的平衡点」作答：key 帮助 React 在 reconcile 时识别哪些元素是「同一个」，决定复用、移动还是销毁
...
围绕「受控组件 vs 非受控组件，性能边界在哪」回答追问时，重点说清 表单 的前提、动作和回退条件。
...
- 追问核心：说明如何验证「受控组件 vs 非受控组件，性能边界在哪」结论成立，给出 表单 的验收路径（对应追问：在当前团队与业务约束下，你会如何结合 表单 指标，避免把「受控组件 vs 非受控组件，性能边界在哪」的实验室提升误判为真实用户体验改善）。
- 直接围绕「在当前团队与业务约束下，你会如何结合 表单 指标，避免把「受控组件 vs 非受控组件，性能边界在哪」的实验室提升误判为真实用户体验改善」作答：受控：value + onChange，组件自身不存储状态，便于校验/联动

#### 落地步骤

- 第一步：回答 受控组件 vs 非受控组件，性能边界在哪 前先给基线：至少明确耗时分位、错误率和用户体感，不然优化动作没有参照。
- 第二步：先定义验收指标，再安排离线用例和线上观测，最后给失败阈值与回滚条件，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 表单 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
- 追问核心：比较「受控组件 vs 非受控组件，性能边界在哪」在收益、成本和维护复杂度上的取舍边界（对应追问：在当前团队与业务约束下，你会怎样评估「受控组件 vs 非受控组件，性能边界在哪」在性能收益与兼容性风险之间的平衡点）。
- 直接围绕「在当前团队与业务约束下，你会怎样评估「受控组件 vs 非受控组件，性能边界在哪」在性能收益与兼容性风险之间的平衡点」作答：受控：value + onChange，组件自身不存储状态，便于校验/联动
...
- 追问核心：围绕「Portal、Error Boundary、Suspense 的协作方式」给出可执行的落地方案，重点说明 架构 怎么做（对应追问：以「Portal、Error Boundary、Suspense 的协作方式」为例，面对团队能力差异，你会如何围绕 架构 把「Portal、Error Boundary、Suspense 的协作方式」拆成可并行推进的小阶段）。
- 直接围绕「以「Portal、Error Boundary、Suspense 的协作方式」为例，面对团队能力差异，你会如何围绕 架构 把「Portal、Error Boundary、Suspense 的协作方式」拆成可并行推进的小阶段」作答：Portal：createPortal(children, document.body)，把子树渲染到任意 DOM，但事件冒泡仍按 React 树

#### 落地步骤

- 第一步：落地 Portal、Error Boundary、Suspense 的协作方式 前要先确认团队协作链路：谁发布、谁观测、谁回退，避免责任空窗。
...
这道追问要直接回应「Portal、Error Boundary、Suspense 的协作方式」在 架构 上的执行动作、风险边界和验收信号。
...
- 追问核心：围绕「Portal、Error Boundary、Suspense 的协作方式」给出可执行的落地方案，重点说明 架构 怎么做（对应追问：结合真实业务约束，半年后要做去留决策时，你会拿哪些数据判断「Portal、Error Boundary、Suspense 的协作方式」还值不值得继续维护）。
- 直接围绕「结合真实业务约束，半年后要做去留决策时，你会拿哪些数据判断「Portal、Error Boundary、Suspense 的协作方式」还值不值得继续维护」作答：Portal：createPortal(children, document.body)，把子树渲染到任意 DOM，但事件冒泡仍按 React 树
...
这道追问要直接回应「React Compiler 自动 memo 的原理、限制与落地边界」在 性能 上的执行动作、风险边界和验收信号。
...
- 追问核心：围绕「React Compiler 自动 memo 的原理、限制与落地边界」给出可执行的落地方案，重点说明 性能 怎么做（对应追问：在「React Compiler 自动 memo 的原理、限制与落地边界」场景下，什么情况下 React Compiler 会跳过某个组件或 hook）。
- 直接围绕「在「React Compiler 自动 memo 的原理、限制与落地边界」场景下，什么情况下 React Compiler 会跳过某个组件或 hook」作答：React Compiler 的核心不是“运行时更聪明”，而是编译阶段识别组件渲染中的稳定表达式、闭包和 JSX 子树，插入等价缓存逻辑，降低父组件重渲染时的无效计算与子树重渲染。

#### 落地步骤

- 第一步：回答 React Compiler 自动 memo 的原理、限制与落地边界 前先给基线：至少明确耗时分位、错误率和用户体感，不然优化动作没有参照。
- 第二步：按“先复现现状 -> 再最小改动实现 -> 最后补回归”推进，确保每一步都可回退，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 性能 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
- 追问核心：说明「React Compiler 自动 memo 的原理、限制与落地边界」上线时如何灰度、观测、回滚（对应追问：以「React Compiler 自动 memo 的原理、限制与落地边界」为例，它和 React.memo、useMemo、useCallback 的迁移顺序怎么安排）。
- 直接围绕「以「React Compiler 自动 memo 的原理、限制与落地边界」为例，它和 React.memo、useMemo、useCallback 的迁移顺序怎么安排」作答：React Compiler 的核心不是“运行时更聪明”，而是编译阶段识别组件渲染中的稳定表达式、闭包和 JSX 子树，插入等价缓存逻辑，降低父组件重渲染时的无效计算与子树重渲染。

#### 落地步骤

- 第一步：React Compiler 自动 memo 的原理、限制与落地边界 只有在瓶颈被数据证实时才值得推进；先确认 性能 是否真是主耗时来源。
...
围绕「React Compiler 自动 memo 的原理、限制与落地边界」回答追问时，重点说清 性能 的前提、动作和回退条件。
...
- 追问核心：说明如何验证「React Compiler 自动 memo 的原理、限制与落地边界」结论成立，给出 性能 的验收路径（对应追问：在当前团队与业务约束下，你会用哪些 Profiler 指标证明 Compiler 真的带来了收益）。
- 直接围绕「在当前团队与业务约束下，你会用哪些 Profiler 指标证明 Compiler 真的带来了收益」作答：React Compiler 的核心不是“运行时更聪明”，而是编译阶段识别组件渲染中的稳定表达式、闭包和 JSX 子树，插入等价缓存逻辑，降低父组件重渲染时的无效计算与子树重渲染。
...
- 追问核心：围绕「React 服务端状态管理：TanStack Query / SWR 缓存失效怎么设计」给出可执行的落地方案，重点说明 React 怎么做（对应追问：结合真实业务约束，staleTime 和 gcTime 分别解决什么问题）。
- 直接围绕「结合真实业务约束，staleTime 和 gcTime 分别解决什么问题」作答：服务端状态有来源权威：真正的数据在服务端，前端只缓存快照；它会过期、被别人修改、需要重新验证。

#### 落地步骤

- 第一步：React 服务端状态管理：TanStack Query / SWR 缓存失效怎么设计 只有在瓶颈被数据证实时才值得推进；先确认 React 是否真是主耗时来源。
- 第二步：先把 React 拆成可执行子步骤，再逐步落地并记录每一步的输入输出，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 React 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
这道追问要直接回应「React 服务端状态管理：TanStack Query / SWR 缓存失效怎么设计」在 React 上的执行动作、风险边界和验收信号。
...
- 追问核心：识别「React 服务端状态管理：TanStack Query / SWR 缓存失效怎么设计」的高风险失败场景并给出兜底措施（对应追问：以「React 服务端状态管理：TanStack Query / SWR 缓存失效怎么设计」为例，乐观更新失败时，如何保证 UI、缓存和服务端最终一致）。
- 直接围绕「以「React 服务端状态管理：TanStack Query / SWR 缓存失效怎么设计」为例，乐观更新失败时，如何保证 UI、缓存和服务端最终一致」作答：服务端状态有来源权威：真正的数据在服务端，前端只缓存快照；它会过期、被别人修改、需要重新验证。

#### 落地步骤

- 第一步：回答 React 服务端状态管理：TanStack Query / SWR 缓存失效怎么设计 前先给基线：至少明确耗时分位、错误率和用户体感，不然优化动作没有参照。
...
- 追问核心：围绕「React 服务端状态管理：TanStack Query / SWR 缓存失效怎么设计」给出可执行的落地方案，重点说明 React 怎么做（对应追问：在「React 服务端状态管理：TanStack Query / SWR 缓存失效怎么设计」场景下，SSR 预取后，如何避免客户端 hydration 又请求一遍）。
- 直接围绕「在「React 服务端状态管理：TanStack Query / SWR 缓存失效怎么设计」场景下，SSR 预取后，如何避免客户端 hydration 又请求一遍」作答：服务端状态有来源权威：真正的数据在服务端，前端只缓存快照；它会过期、被别人修改、需要重新验证。
...
- 追问核心：解释「useSyncExternalStore 如何保证外部 Store 与并发渲染一致性」背后的因果关系，并指出 React 的触发条件（对应追问：tearing 是什么，为什么 React 并发渲染会放大这个问题）。
- 直接围绕「tearing 是什么，为什么 React 并发渲染会放大这个问题」作答：外部 store 不受 React 管理，如果组件在渲染过程中 store 变化，可能出现同一屏不同组件读到不同版本的状态，这就是 tearing。

#### 落地步骤

- 第一步：useSyncExternalStore 如何保证外部 Store 与并发渲染一致性 只有在瓶颈被数据证实时才值得推进；先确认 React 是否真是主耗时来源。
- 第二步：围绕 useSyncExternalStore 如何保证外部 Store 与并发渲染一致性 用“前提 -> 机制 -> 失效场景”三段式解释，保证因果闭环，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 React 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
这道追问要直接回应「useSyncExternalStore 如何保证外部 Store 与并发渲染一致性」在 React 上的执行动作、风险边界和验收信号。
...
- 追问核心：解释「useSyncExternalStore 如何保证外部 Store 与并发渲染一致性」背后的因果关系，并指出 React 的触发条件（对应追问：在当前团队与业务约束下，getSnapshot 的返回值为什么需要引用稳定）。
- 直接围绕「在当前团队与业务约束下，getSnapshot 的返回值为什么需要引用稳定」作答：外部 store 不受 React 管理，如果组件在渲染过程中 store 变化，可能出现同一屏不同组件读到不同版本的状态，这就是 tearing。

#### 落地步骤

- 第一步：回答 useSyncExternalStore 如何保证外部 Store 与并发渲染一致性 前先给基线：至少明确耗时分位、错误率和用户体感，不然优化动作没有参照。
- 第二步：先说触发条件，再解释机制，再给反例，避免把“结果”当成“原因”，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 React 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
- 追问核心：解释「useSyncExternalStore 如何保证外部 Store 与并发渲染一致性」背后的因果关系，并指出 React 的触发条件（对应追问：以「useSyncExternalStore 如何保证外部 Store 与并发渲染一致性」为例，Zustand / Redux 这类库在 React 18 之后为什么要适配这个 Hook）。
- 直接围绕「以「useSyncExternalStore 如何保证外部 Store 与并发渲染一致性」为例，Zustand / Redux 这类库在 React 18 之后为什么要适配这个 Hook」作答：外部 store 不受 React 管理，如果组件在渲染过程中 store 变化，可能出现同一屏不同组件读到不同版本的状态，这就是 tearing。
...
- 追问核心：解释「React 复杂表单：校验、异步默认值、联动和提交状态怎么设计」背后的因果关系，并指出 React 的触发条件（对应追问：在当前团队与业务约束下，React Hook Form 为什么在大表单里通常比全受控 state 更省渲染）。
- 直接围绕「在当前团队与业务约束下，React Hook Form 为什么在大表单里通常比全受控 state 更省渲染」作答：简单表单可用受控组件；字段多、联动多、性能敏感时，React Hook Form 这类基于 uncontrolled + subscription 的方案更适合。

#### 落地步骤

- 第一步：React 复杂表单：校验、异步默认值、联动和提交状态怎么设计 只有在瓶颈被数据证实时才值得推进；先确认 React 是否真是主耗时来源。
- 第二步：围绕 React 复杂表单：校验、异步默认值、联动和提交状态怎么设计 用“前提 -> 机制 -> 失效场景”三段式解释，保证因果闭环，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 React 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。

#### 风险与验收

- 失败风险：常见失败是只优化实验室分数，真实设备或弱网下 React 复杂表单：校验、异步默认值、联动和提交状态怎么设计 体验反而抖动。
- 验收信号：验收至少包含真实用户指标、分位耗时和错误率对照，而非单一跑分。
...
- 追问核心：围绕「React 复杂表单：校验、异步默认值、联动和提交状态怎么设计」给出可执行的落地方案，重点说明 React 怎么做（对应追问：异步唯一性校验如何避免慢响应覆盖新输入）。
- 直接围绕「异步唯一性校验如何避免慢响应覆盖新输入」作答：简单表单可用受控组件；字段多、联动多、性能敏感时，React Hook Form 这类基于 uncontrolled + subscription 的方案更适合。

#### 落地步骤

- 第一步：回答 React 复杂表单：校验、异步默认值、联动和提交状态怎么设计 时要说明 React 在并发渲染下的行为差异和回归策略。
- 第二步：先把 React 拆成可执行子步骤，再逐步落地并记录每一步的输入输出，并且动作要交代渲染边界、状态分层和失效策略。
- 第三步：如果 React 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
围绕「React 复杂表单：校验、异步默认值、联动和提交状态怎么设计」回答追问时，重点说清 React 的前提、动作和回退条件。
...
- 追问核心：围绕「React 复杂表单：校验、异步默认值、联动和提交状态怎么设计」给出可执行的落地方案，重点说明 React 怎么做（对应追问：在当前团队与业务约束下，分步表单里，草稿保存和最终提交校验如何协调）。
- 直接围绕「在当前团队与业务约束下，分步表单里，草稿保存和最终提交校验如何协调」作答：简单表单可用受控组件；字段多、联动多、性能敏感时，React Hook Form 这类基于 uncontrolled + subscription 的方案更适合。

#### 落地步骤

- 第一步：React 复杂表单：校验、异步默认值、联动和提交状态怎么设计 要先拆分状态来源：本地状态、缓存状态、路由状态边界不能混用。
...
围绕「Suspense 与异步数据加载」回答追问时，重点说清 Suspense 的前提、动作和回退条件。
...
- 追问核心：说明如何验证「Suspense 与异步数据加载」结论成立，给出 Suspense 的验收路径（对应追问：为了避免主观判断，你会怎样用测试证据和线上指标共同证明 Suspense 方案有效）。
- 直接围绕「为了避免主观判断，你会怎样用测试证据和线上指标共同证明 Suspense 方案有效」作答：Suspense 让组件在数据未就绪时"挂起"，由父级 fallback 渲染 spinner，无需手写 isLoading 分支

#### 落地步骤

- 第一步：回答 Suspense 与异步数据加载 时先定义验收口径：主路径、边界输入和失败分支都要有可复现用例。
- 第二步：先定义验收指标，再安排离线用例和线上观测，最后给失败阈值与回滚条件，并且动作要覆盖边界输入、时序异常和回归用例，避免“只测主路径”。
- 第三步：如果 Suspense 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
- 追问核心：围绕「Suspense 与异步数据加载」给出可执行的落地方案，重点说明 Suspense 怎么做（对应追问：从工程落地角度看，如果兼容性压力突然升高，你会如何围绕 Suspense 重新划分「Suspense 与异步数据加载」的实施阶段）。
- 直接围绕「从工程落地角度看，如果兼容性压力突然升高，你会如何围绕 Suspense 重新划分「Suspense 与异步数据加载」的实施阶段」作答：Suspense 让组件在数据未就绪时"挂起"，由父级 fallback 渲染 spinner，无需手写 isLoading 分支

#### 落地步骤

- 第一步：Suspense 与异步数据加载 的前置条件是迁移批次、灰度门槛和回滚策略明确，不然执行风险不可控。
- 第二步：按“先复现现状 -> 再最小改动实现 -> 最后补回归”推进，确保每一步都可回退，并且动作要同时交代迁移批次、灰度策略和回滚门槛。
- 第三步：如果 Suspense 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
- 实施步骤：把验证拆成“可复现样例 + 指标监控 + 告警阈值”，确保结论可复核，并且动作要交代渲染边界、状态分层和失效策略。
- 失败风险：常见风险是状态源混用，出现重复请求、脏读或 UI 闪烁。
- 验收信号：验收至少看渲染次数、请求重复率和状态一致性告警。
...
- 追问核心：说明「React 渲染预算治理：把重渲染风险前置到发布闸门」上线时如何灰度、观测、回滚（对应追问：结合真实业务约束，如果要做「React 渲染预算治理：把重渲染风险前置到发布闸门」的稳定性评审，你会先盘点哪些容易被忽视的边界因素）。
- 直接围绕「结合真实业务约束，如果要做「React 渲染预算治理：把重渲染风险前置到发布闸门」的稳定性评审，你会先盘点哪些容易被忽视的边界因素」作答：先定义预算对象：关键页面渲染次数、提交耗时分位、长任务占比、交互延迟阈值。

#### 落地步骤

- 第一步：React 渲染预算治理：把重渲染风险前置到发布闸门 只有在瓶颈被数据证实时才值得推进；先确认 React 是否真是主耗时来源。
- 第二步：发布按批次推进：每批次都有观测窗口、验收条件和回退动作，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 React 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
围绕「React 渲染预算治理：把重渲染风险前置到发布闸门」回答追问时，重点说清 React 的前提、动作和回退条件。
...
- 追问核心：说明如何验证「React 渲染预算治理：把重渲染风险前置到发布闸门」结论成立，给出 React 的验收路径（对应追问：从工程落地角度看，上线后你会盯哪些与 React 相关的日志与指标，来确认这套方案确实带来改进）。
- 直接围绕「从工程落地角度看，上线后你会盯哪些与 React 相关的日志与指标，来确认这套方案确实带来改进」作答：先定义预算对象：关键页面渲染次数、提交耗时分位、长任务占比、交互延迟阈值。

#### 落地步骤

- 第一步：回答 React 渲染预算治理：把重渲染风险前置到发布闸门 前先给基线：至少明确耗时分位、错误率和用户体感，不然优化动作没有参照。
- 第二步：先定义验收指标，再安排离线用例和线上观测，最后给失败阈值与回滚条件，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 React 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
- 追问核心：比较「React 渲染预算治理：把重渲染风险前置到发布闸门」在收益、成本和维护复杂度上的取舍边界（对应追问：在「React 渲染预算治理：把重渲染风险前置到发布闸门」场景下，如果「React 渲染预算治理：把重渲染风险前置到发布闸门」在 React 上的收益和维护成本打架，你会怎么做取舍判断）。
- 直接围绕「在「React 渲染预算治理：把重渲染风险前置到发布闸门」场景下，如果「React 渲染预算治理：把重渲染风险前置到发布闸门」在 React 上的收益和维护成本打架，你会怎么做取舍判断」作答：先定义预算对象：关键页面渲染次数、提交耗时分位、长任务占比、交互延迟阈值。
...
- 追问核心：说明如何验证「React 水合不一致排障手册：从告警定位到稳定回退」结论成立，给出 React 的验收路径（对应追问：从工程落地角度看，在 React 项目里应用「React 水合不一致排障手册：从告警定位到稳定回退」时，哪些 state 或渲染边界最容易出问题）。
- 直接围绕「从工程落地角度看，在 React 项目里应用「React 水合不一致排障手册：从告警定位到稳定回退」时，哪些 state 或渲染边界最容易出问题」作答：先按类型分流：结构不一致、数据不一致、环境差异（时区/随机数/客户端 API）分别处理。

#### 落地步骤

- 第一步：React 水合不一致排障手册：从告警定位到稳定回退 只有在瓶颈被数据证实时才值得推进；先确认 React 是否真是主耗时来源。
- 第二步：把验证拆成“可复现样例 + 指标监控 + 告警阈值”，确保结论可复核，并且动作执行后要验证主线程时长、慢请求比例和核心体验指标是否同步改善。
- 第三步：如果 React 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。

#### 风险与验收

- 失败风险：常见失败是只优化实验室分数，真实设备或弱网下 React 水合不一致排障手册：从告警定位到稳定回退 体验反而抖动。
- 验收信号：验收至少包含真实用户指标、分位耗时和错误率对照，而非单一跑分。
...
这道追问要直接回应「React 水合不一致排障手册：从告警定位到稳定回退」在 React 上的执行动作、风险边界和验收信号。
...
- 追问核心：说明如何验证「React 水合不一致排障手册：从告警定位到稳定回退」结论成立，给出 React 的验收路径（对应追问：为了避免主观判断，你会怎样用测试证据和线上指标共同证明 React 方案有效）。
- 直接围绕「为了避免主观判断，你会怎样用测试证据和线上指标共同证明 React 方案有效」作答：先按类型分流：结构不一致、数据不一致、环境差异（时区/随机数/客户端 API）分别处理。

#### 落地步骤

- 第一步：回答 React 水合不一致排障手册：从告警定位到稳定回退 时先定义验收口径：主路径、边界输入和失败分支都要有可复现用例。
- 第二步：先定义验收指标，再安排离线用例和线上观测，最后给失败阈值与回滚条件，并且动作要覆盖边界输入、时序异常和回归用例，避免“只测主路径”。
- 第三步：如果 React 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
围绕「React 水合不一致排障手册：从告警定位到稳定回退」回答追问时，重点说清 React 的前提、动作和回退条件。
...
- 追问核心：说明如何验证「React 水合不一致排障手册：从告警定位到稳定回退」结论成立，给出 React 的验收路径（对应追问：从工程落地角度看，如果比较「React 水合不一致排障手册：从告警定位到稳定回退」与替代方案，你会如何基于 React 判断不同团队阶段的最佳选择）。
- 直接围绕「从工程落地角度看，如果比较「React 水合不一致排障手册：从告警定位到稳定回退」与替代方案，你会如何基于 React 判断不同团队阶段的最佳选择」作答：先按类型分流：结构不一致、数据不一致、环境差异（时区/随机数/客户端 API）分别处理。

#### 落地步骤

- 第一步：React 水合不一致排障手册：从告警定位到稳定回退 的前置条件是迁移批次、灰度门槛和回滚策略明确，不然执行风险不可控。
- 第二步：先定义验收指标，再安排离线用例和线上观测，最后给失败阈值与回滚条件，并且动作要同时交代迁移批次、灰度策略和回滚门槛。
- 第三步：如果 React 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
围绕「React Compiler 升级指挥桥：收益验证、风险分层与一键止损」回答追问时，重点说清 发布治理 的前提、动作和回退条件。
...
- 追问核心：说明「React Compiler 升级指挥桥：收益验证、风险分层与一键止损」上线时如何灰度、观测、回滚（对应追问：React Compiler 真正上线前，你会先验哪些高风险边界，避免“测得好、线上抖”）。
- 直接围绕「React Compiler 真正上线前，你会先验哪些高风险边界，避免“测得好、线上抖”」作答：先做风险分层：按页面复杂度、交互密度、历史事故频率拆出高/中/低风险域。

#### 落地步骤

- 第一步：React Compiler 升级指挥桥：收益验证、风险分层与一键止损 的前置条件是迁移批次、灰度门槛和回滚策略明确，不然执行风险不可控。
- 第二步：先选低风险流量灰度，再按指标放量，异常时按预案快速回滚，并且动作要同时交代迁移批次、灰度策略和回滚门槛。
...
- 追问核心：说明如何验证「React Compiler 升级指挥桥：收益验证、风险分层与一键止损」结论成立，给出 发布治理 的验收路径（对应追问：你说 Compiler 升级有收益，那你怎么定义“真的生效”，并持续验证不是偶然波动）。
- 直接围绕「你说 Compiler 升级有收益，那你怎么定义“真的生效”，并持续验证不是偶然波动」作答：先做风险分层：按页面复杂度、交互密度、历史事故频率拆出高/中/低风险域。

#### 落地步骤

- 第一步：落地 React Compiler 升级指挥桥：收益验证、风险分层与一键止损 前要先确认团队协作链路：谁发布、谁观测、谁回退，避免责任空窗。
...
这道追问要直接回应「React Compiler 升级指挥桥：收益验证、风险分层与一键止损」在 发布治理 上的执行动作、风险边界和验收信号。
...
- 追问核心：围绕「React Compiler 升级指挥桥：收益验证、风险分层与一键止损」给出可执行的落地方案，重点说明 发布治理 怎么做（对应追问：业务复杂度差异很大时，你会怎么分层启用 React Compiler，而不是全站一把梭）。
- 直接围绕「业务复杂度差异很大时，你会怎么分层启用 React Compiler，而不是全站一把梭」作答：先做风险分层：按页面复杂度、交互密度、历史事故频率拆出高/中/低风险域。
...
围绕「React 乐观更新止损手册：误成功反馈、重复提交与回滚沟通」回答追问时，重点说清 React 19 的前提、动作和回退条件。
...
- 追问核心：围绕「React 乐观更新止损手册：误成功反馈、重复提交与回滚沟通」给出可执行的落地方案，重点说明 React 19 怎么做（对应追问：这套乐观更新方案在真实流量下最容易在哪些场景失效，你会怎么判断）。
- 直接围绕「这套乐观更新方案在真实流量下最容易在哪些场景失效，你会怎么判断」作答：先切断扩散：临时关闭高风险乐观路径，优先恢复“真实写后再展示”的保守策略。

#### 落地步骤

- 第一步：React 乐观更新止损手册：误成功反馈、重复提交与回滚沟通 的前置条件是迁移批次、灰度门槛和回滚策略明确，不然执行风险不可控。
- 第二步：按“先复现现状 -> 再最小改动实现 -> 最后补回归”推进，确保每一步都可回退，并且动作要同时交代迁移批次、灰度策略和回滚门槛。
- 第三步：如果 React 19 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
这道追问要直接回应「React 乐观更新止损手册：误成功反馈、重复提交与回滚沟通」在 React 19 上的执行动作、风险边界和验收信号。
...
- 追问核心：说明如何验证「React 乐观更新止损手册：误成功反馈、重复提交与回滚沟通」结论成立，给出 React 19 的验收路径（对应追问：你这套乐观更新止损策略不是纸面方案，你会怎样用测试和线上数据证明它有效）。
- 直接围绕「你这套乐观更新止损策略不是纸面方案，你会怎样用测试和线上数据证明它有效」作答：先切断扩散：临时关闭高风险乐观路径，优先恢复“真实写后再展示”的保守策略。

#### 落地步骤

- 第一步：回答 React 乐观更新止损手册：误成功反馈、重复提交与回滚沟通 时先定义验收口径：主路径、边界输入和失败分支都要有可复现用例。
- 第二步：先定义验收指标，再安排离线用例和线上观测，最后给失败阈值与回滚条件，并且动作要覆盖边界输入、时序异常和回归用例，避免“只测主路径”。
- 第三步：如果 React 19 链路出现异常，先降级保护核心功能，再按预案回退并记录复盘结论。
...
- 追问核心：比较「React 乐观更新止损手册：误成功反馈、重复提交与回滚沟通」在收益、成本和维护复杂度上的取舍边界（对应追问：团队从小团队扩到多团队协作后，你会怎么调整乐观更新策略，避免治理成本失控）。
- 直接围绕「团队从小团队扩到多团队协作后，你会怎么调整乐观更新策略，避免治理成本失控」作答：先切断扩散：临时关闭高风险乐观路径，优先恢复“真实写后再展示”的保守策略。
