...
followups: [hashing-cache-followup-1]
difficulty: 基础
tags: [缓存, 发布]
...
- 如果把「hash 命名、长效缓存与 HTML 短缓存是发布基础功」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
followups: [chunk-failure-followup-1]
difficulty: 进阶
tags: [动态加载, 容错]
...
- 如果把「动态 import 失败与旧版本 chunk 被清理怎么处理」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 灰度发布、回滚与零停机切换
followups: [gray-release-followup-1]
difficulty: 进阶
tags: [灰度, 回滚]
...
- 如果把「灰度发布、回滚与零停机切换」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
followups: [service-worker-update-followup-1]
difficulty: 进阶
...
- 如果把「Service Worker 更新策略的取舍」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
followups: [spa-fallback-followup-1]
difficulty: 基础
tags: [路由, 静态部署]
...
- 如果把「history 路由、404 fallback 与静态托管适配」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 包体分析与发布前治理
followups: [bundle-governance-followup-1]
difficulty: 进阶
tags: [包体治理, 分析]
...
- 如果把「包体分析与发布前治理」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
followups: [tree-shaking-deep-followup-1]
difficulty: 进阶
...
- 如果把「Tree-shaking 失效的常见原因」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
followups: [sw-update-strategies-followup-1]
difficulty: 资深
...
- 如果把「PWA Service Worker 升级策略」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
followups: [semver-release-followup-1]
difficulty: 进阶
tags: [发布, 工程化]
...
- 如果把「SemVer 与自动化发版（changeset / semantic-release）」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
followups: [ci-cd-frontend-pipeline-followup-1]
difficulty: 资深
tags: [CI/CD, 工程化, 高频]
...
- 如果把「前端 CI/CD 流水线怎么设计」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 一道题打包优化全部场景
followups: [bundle-optimization-tactics-followup-1]
difficulty: 资深
tags: [构建, 性能, 高频]
...
- 如果把「一道题打包优化全部场景」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「hash 命名、长效缓存与 HTML 短缓存是发布基础功」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 基础
tags: [缓存, 发布, 追问]
...
如果面试官追问：如果把「hash 命名、长效缓存与 HTML 短缓存是发布基础功」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「动态 import 失败与旧版本 chunk 被清理怎么处理」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [动态加载, 容错, 追问]
...
如果面试官追问：如果把「动态 import 失败与旧版本 chunk 被清理怎么处理」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「灰度发布、回滚与零停机切换」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [灰度, 回滚, 追问]
parent: gray-release
...
如果面试官追问：如果把「灰度发布、回滚与零停机切换」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Service Worker 更新策略的取舍」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
...
如果面试官追问：如果把「Service Worker 更新策略的取舍」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「history 路由、404 fallback 与静态托管适配」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 基础
tags: [路由, 静态部署, 追问]
parent: spa-fallback
...
如果面试官追问：如果把「history 路由、404 fallback 与静态托管适配」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「包体分析与发布前治理」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [包体治理, 分析, 追问]
...
如果面试官追问：如果把「包体分析与发布前治理」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 关注包预算（performance budget）：在 CI 里设置阈值（如主 chunk gzip < 200KB），超出报错
- 治理动作：懒加载、按需导入、Tree-shaking 友好的写法、polyfill 收敛
- 很多包体问题不是"代码多"，而是依赖接入方式不对
- 可以补充一个真实项目语境：上线前先约定输入输出、失败兜底和观测指标，避免只在 demo 场景下成立。
...
title: 追问：如果把「Tree-shaking 失效的常见原因」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
...
如果面试官追问：如果把「Tree-shaking 失效的常见原因」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 先把问题拉回「Tree-shaking 失效的常见原因」的核心机制，说明这个追问考察的是落地边界、失败条件和方案取舍，而不是单点定义。
- 可以补充一个真实项目语境：上线前先约定输入输出、失败兜底和观测指标，避免只在 demo 场景下成立。
...
title: 追问：如果把「PWA Service Worker 升级策略」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
...
如果面试官追问：如果把「PWA Service Worker 升级策略」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「SemVer 与自动化发版」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [发布, 工程化, 追问]
...
如果面试官追问：如果把「SemVer 与自动化发版（changeset / semantic-release）」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「前端 CI/CD 流水线怎么设计」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
...
如果面试官追问：如果把「前端 CI/CD 流水线怎么设计」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 先把问题拉回「前端 CI/CD 流水线怎么设计」的核心机制，说明这个追问考察的是落地边界、失败条件和方案取舍，而不是单点定义。
- 可以补充一个真实项目语境：上线前先约定输入输出、失败兜底和观测指标，避免只在 demo 场景下成立。
...
title: 追问：如果把「一道题打包优化全部场景」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [构建, 性能, 高频, 追问]
...
如果面试官追问：如果把「一道题打包优化全部场景」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
