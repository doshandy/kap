...
title: 追问：如果把「XSS 三种类型与前端最该做的防御」放到真实业务里，你会怎么划分信任边界和服务端兜底
difficulty: 基础
tags: [XSS, 输出编码, 追问]
parent: xss
...
如果面试官追问：如果把「XSS 三种类型与前端最该做的防御」放到真实业务里，你会怎么划分信任边界和服务端兜底？
...
title: 追问：「CSP 与 Trusted Types 为什么是现代前端的高阶防线」在真实项目里最容易踩到哪些边界条件
difficulty: 进阶
...
如果面试官追问：「CSP 与 Trusted Types 为什么是现代前端的高阶防线」在真实项目里最容易踩到哪些边界条件？

### 答案要点

#### 核心回答

- 先界定「CSP 与 Trusted Types 为什么是现代前端的高阶防线」的适用条件，再说明哪些输入、运行环境或团队约束会让默认方案失效。
- 围绕核心机制展开：正常路径要能讲清楚，异常路径要说明降级、替代方案和用户可见影响。
- 原题中的关键点「CSP 可以限制脚本来源、禁止内联脚本、配合 nonce/hash 管控执行入口」要进一步补到边界条件里，而不是只复述结论。

## csrf-clickjacking-followup-1

title: 追问：如果把「CSRF、点击劫持与 SameSite 的关系」放到真实业务里，你会怎么划分信任边界和服务端兜底
difficulty: 基础
...
如果面试官追问：如果把「CSRF、点击劫持与 SameSite 的关系」放到真实业务里，你会怎么划分信任边界和服务端兜底？
...
title: 追问：如果把「CORS、OAuth、JWT 是三回事，别混着讲」放到真实业务里，你会怎么划分信任边界和服务端兜底
difficulty: 进阶
...
如果面试官追问：如果把「CORS、OAuth、JWT 是三回事，别混着讲」放到真实业务里，你会怎么划分信任边界和服务端兜底？
...
title: 追问：如果把「npm 供应链攻击与前端依赖治理」放到真实业务里，你会怎么划分信任边界和服务端兜底
difficulty: 进阶
tags: [供应链安全, npm, 追问]
parent: supply-chain
...
如果面试官追问：如果把「npm 供应链攻击与前端依赖治理」放到真实业务里，你会怎么划分信任边界和服务端兜底？
...
title: 追问：「原型链污染为什么危险，如何防」在真实项目里最容易踩到哪些边界条件
difficulty: 进阶
tags: [原型链污染, 对象合并, 追问]
...
如果面试官追问：「原型链污染为什么危险，如何防」在真实项目里最容易踩到哪些边界条件？
...
- 围绕核心机制展开：正常路径要能讲清楚，异常路径要说明降级、替代方案和用户可见影响。
- 原题中的关键点「攻击者通过 **proto**、constructor.prototype 等路径污染全局原型」要进一步补到边界条件里，而不是只复述结论。

## source-map-secrets-followup-1

title: 追问：「Source Map、环境变量与前端敏感信息边界」在真实项目里最容易踩到哪些边界条件
difficulty: 基础
...
如果面试官追问：「Source Map、环境变量与前端敏感信息边界」在真实项目里最容易踩到哪些边界条件？
...
title: 追问：如果把「Passkeys / WebAuthn 取代密码的工程化路径」放到真实业务里，你会怎么划分信任边界和服务端兜底
difficulty: 资深
...
如果面试官追问：如果把「Passkeys / WebAuthn 取代密码的工程化路径」放到真实业务里，你会怎么划分信任边界和服务端兜底？
...
title: 追问：「Subresource Integrity 与第三方资源篡改」在弱网、代理、断连或服务端限流时会出现哪些边界问题
difficulty: 进阶
tags: [SRI, CDN, 追问]
...
如果面试官追问：「Subresource Integrity 与第三方资源篡改」在弱网、代理、断连或服务端限流时会出现哪些边界问题？
...
title: 追问：如果把「XSS 与 CSRF 的区别和防御」放到真实业务里，你会怎么划分信任边界和服务端兜底
difficulty: 进阶
...
如果面试官追问：如果把「XSS 与 CSRF 的区别和防御」放到真实业务里，你会怎么划分信任边界和服务端兜底？
...
title: 追问：如果把「鉴权方案 Cookie+Session vs JWT 怎么选」放到真实业务里，你会怎么划分信任边界和服务端兜底
difficulty: 进阶
...
如果面试官追问：如果把「鉴权方案 Cookie+Session vs JWT 怎么选」放到真实业务里，你会怎么划分信任边界和服务端兜底？
...
title: 追问：如果把「前端供应链攻击怎么防」放到真实业务里，你会怎么划分信任边界和服务端兜底
difficulty: 资深
tags: [安全, 供应链, 高频, 追问]
...
如果面试官追问：如果把「前端供应链攻击怎么防」放到真实业务里，你会怎么划分信任边界和服务端兜底？
...
title: 追问：如果把「浏览器原生 Web Crypto API 怎么用？哈希 / 对称 / 非对称 / 签名场景速查」放到真实业务里，你会怎么划分信任边界和服务端兜底
difficulty: 进阶
...
如果面试官追问：如果把「浏览器原生 Web Crypto API 怎么用？哈希 / 对称 / 非对称 / 签名场景速查」放到真实业务里，你会怎么划分信任边界和服务端兜底？
...
title: 追问：如果把「客服 / SaaS 场景里前端常被忽视的"敏感信息泄漏面"有哪些」放到真实业务里，你会怎么划分信任边界和服务端兜底
difficulty: 进阶
...
如果面试官追问：如果把「客服 / SaaS 场景里前端常被忽视的"敏感信息泄漏面"有哪些」放到真实业务里，你会怎么划分信任边界和服务端兜底？
