...
- 如果把「async/await 在事件循环里的真实执行顺序」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Proxy 与 Reflect：13 种 trap 与 receiver 正确性」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「迭代器、生成器与异步迭代」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「结构化克隆 vs JSON 序列化」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「模板字符串与标签模板的实战」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「WeakMap / WeakSet / WeakRef 与垃圾回收」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「bind / call / apply 的区别与手写实现」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「new 操作符做了哪些事，怎么手写」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「Promise.all / allSettled / race / any 的差异和典型用法」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「数组扁平化的多种实现」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「手写一个 EventEmitter（订阅发布）」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 如果把「防抖（debounce）的 immediate 模式怎么实现」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：为什么 Object.create 创建的对象更适合做「纯字典」
difficulty: 基础
tags: [原型, 继承, 追问]
...
如果面试官追问：为什么 `Object.create(null)` 创建的对象更适合做「纯字典」？
...
如果面试官追问：修改原型链（`Object.setPrototypeOf`）为什么是性能反模式？
...
title: 追问：严格模式下默认绑定的 this 是什么？为什么这么设计
difficulty: 基础
tags: [this, 函数, 追问]
parent: this-binding
...
如果面试官追问：严格模式下默认绑定的 this 是什么？为什么这么设计
...
title: 追问：Node 的事件循环和浏览器有什么差异？setImmediate vs setTimeout
difficulty: 进阶
tags: [事件循环, 异步, 追问]
parent: event-loop
...
如果面试官追问：Node 的事件循环和浏览器有什么差异？setImmediate vs setTimeout
...
title: 追问：queueMicrotask、Promise.resolve.then 哪个先执行
difficulty: 进阶
tags: [事件循环, 异步, 追问]
parent: event-loop
...
如果面试官追问：`queueMicrotask`、`Promise.resolve().then` 哪个先执行
...
title: 追问：async/await 在 Event Loop 中的实际表现
difficulty: 进阶
tags: [事件循环, 异步, 追问]
parent: event-loop
...
如果面试官追问：async/await 在 Event Loop 中的实际表现（每个 await 等价于一次微任务调度）
...
如果面试官追问：实现一个简易 Promise.all（已在算法专题里）
...
title: 追问：为什么说 async/await 是 Promise 的语法糖？有什么差异
difficulty: 进阶
tags: [Promise, 异步, 手写, 追问]
parent: promise-aplus
...
如果面试官追问：为什么说 async/await 是 Promise 的语法糖？有什么差异（堆栈、异常）
...
title: 追问：如果把「async/await 在事件循环里的真实执行顺序」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [async, 事件循环, 追问]
parent: async-await
...
如果面试官追问：如果把「async/await 在事件循环里的真实执行顺序」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
- 先把问题拉回「async/await 在事件循环里的真实执行顺序」的核心机制，说明这个追问考察的是落地边界、失败条件和方案取舍，而不是单点定义。
- 可以补充一个真实项目语境：上线前先约定输入输出、失败兜底和观测指标，避免只在 demo 场景下成立。
...
title: 追问：如何实现「前沿+后沿」都触发的 throttle
difficulty: 基础
tags: [手写, 性能, 追问]
...
如果面试官追问：如何实现「前沿+后沿」都触发的 throttle
...
title: 追问：requestAnimationFrame 实现的 throttle 和 setTimeout 实现有何差异
difficulty: 基础
tags: [手写, 性能, 追问]
parent: debounce-throttle
...
如果面试官追问：requestAnimationFrame 实现的 throttle 和 setTimeout 实现有何差异
...
title: 追问：React 里如何用 useRef 实现稳定的 debounce 函数
difficulty: 基础
tags: [手写, 性能, 追问]
parent: debounce-throttle
...
如果面试官追问：React 里如何用 useRef 实现稳定的 debounce 函数（避免每次 render 重新创建）
...
title: 追问：structuredClone 和 lodash cloneDeep 哪个全？哪个快
difficulty: 进阶
tags: [手写, 对象, 追问]
parent: deep-clone
...
如果面试官追问：structuredClone 和 lodash cloneDeep 哪个全？哪个快
...
- 先把问题拉回「深拷贝完整实现：循环引用、Symbol、特殊对象」的核心机制，说明这个追问考察的是落地边界、失败条件和方案取舍，而不是单点定义。
- 可以补充一个真实项目语境：上线前先约定输入输出、失败兜底和观测指标，避免只在 demo 场景下成立。
...
如果面试官追问：写一个支持循环引用的简易深拷贝（用 WeakMap 备忘）
...
title: 追问：为什么 ESM 能 tree-shake，CJS 通常不能
difficulty: 进阶
tags: [模块化, Node, 追问]
parent: esm-vs-cjs
...
如果面试官追问：为什么 ESM 能 tree-shake，CJS 通常不能
...
title: 追问：import 一个 CJS 模块时 default 怎么映射
difficulty: 进阶
tags: [模块化, Node, 追问]
parent: esm-vs-cjs
...
如果面试官追问：import 一个 CJS 模块时 default 怎么映射
...
如果面试官追问：排查内存泄漏的标准流程（三次 Heap snapshot 对比）
...
title: 追问：React 里 useEffect 没 return cleanup 会怎样
difficulty: 进阶
tags: [内存, 性能, 追问]
parent: memory-leak
...
如果面试官追问：React 里 useEffect 没 return cleanup 会怎样
...
- 先把问题拉回「前端常见内存泄漏与排查」的核心机制，说明这个追问考察的是落地边界、失败条件和方案取舍，而不是单点定义。
- 可以补充一个真实项目语境：上线前先约定输入输出、失败兜底和观测指标，避免只在 demo 场景下成立。
...
title: 追问：如果把「Proxy 与 Reflect：13 种 trap 与 receiver 正确性」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [Proxy, 元编程, 追问]
...
如果面试官追问：如果把「Proxy 与 Reflect：13 种 trap 与 receiver 正确性」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「迭代器、生成器与异步迭代」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [迭代器, 生成器, 追问]
...
如果面试官追问：如果把「迭代器、生成器与异步迭代」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「结构化克隆 vs JSON 序列化」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
...
如果面试官追问：如果把「结构化克隆 vs JSON 序列化」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「模板字符串与标签模板的实战」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [模板字符串, 标签模板, 追问]
...
如果面试官追问：如果把「模板字符串与标签模板的实战」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「WeakMap / WeakSet / WeakRef 与垃圾回收」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 资深
tags: [WeakMap, GC, 追问]
...
如果面试官追问：如果把「WeakMap / WeakSet / WeakRef 与垃圾回收」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「bind / call / apply 的区别与手写实现」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [this, 函数, 追问]
...
如果面试官追问：如果把「bind / call / apply 的区别与手写实现」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「new 操作符做了哪些事，怎么手写」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [对象, 函数, 追问]
parent: new-operator
...
如果面试官追问：如果把「new 操作符做了哪些事，怎么手写」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「Promise.all / allSettled / race / any 的差异和典型用法」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [异步, Promise, 追问]
...
如果面试官追问：如果把「Promise.all / allSettled / race / any 的差异和典型用法」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「数组扁平化的多种实现」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 基础
tags: [数组, 手写, 追问]
...
如果面试官追问：如果把「数组扁平化的多种实现」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「手写一个 EventEmitter」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 基础
tags: [设计模式, 手写, 追问]
...
如果面试官追问：如果把「手写一个 EventEmitter（订阅发布）」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
...
title: 追问：如果把「防抖的 immediate 模式怎么实现」用到真实项目里，你会重点关注哪些边界、验证手段和取舍
difficulty: 进阶
tags: [手写, 性能, 追问]
...
如果面试官追问：如果把「防抖（debounce）的 immediate 模式怎么实现」用到真实项目里，你会重点关注哪些边界、验证手段和取舍？
